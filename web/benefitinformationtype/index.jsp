...
<html lang="en">
    ...
    <%
            ...
            int count = 0;
            int recordsperpage = benefitinformationtype.getCount();
            ...
            int showsizelist = benefitinformationtype.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <head>
        <meta charset="utf-8">
        <title><%= benefitinformationtype.getMainPath("title") != null ? benefitinformationtype.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/benefitinformationtype.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/benefitinformationtype/BenefitinformationtypeAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="benefitinformationtypeId"/>
            <!-- Begin page -->
                                ...
                                <div class="col-md-12 col-xl-12">
                                    <div class="body-background mt_0">
                                        <div class="row" id='ajax_cat'>
                                            ...
                                            <%
                                                                                    int benefitinformationtypeCount = count;
                                                                                    int nextCount = 0;
                                                                                    String prev = "";
                                                                                    String prevclose = "";
                                                                                    String next = "";
                                            ...
                                            <%
                                                                                        if (nextCount == 0 && benefitinformationtypeCount > recordsperpage)
                                                                                        {
                                                                                            next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                                                                                            nextclose = ("</i></a></li>");
                                                                                        }
                                                                                        else if (nextCount > 0 && benefitinformationtypeCount >
                                                                                            ...
                                                                                            ((nextCount*recordsperpage) + recordsperpage))
                                                                                        {
                                                                                            prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                                                            prevclose = ("</i></a></li>");
                                                                                            last = benefitinformationtypeCount;
                                                                                        }
                                                                                        else
                                                                                        {
                                                                                            recordsperpage = benefitinformationtypeCount;
                                                                                        }
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                    ...
                                                                                    if (recordsperpage > 0)
                                                                                    {
                                                                                        noOfPages = benefitinformationtypeCount / recordsperpage;
                                                                                        if (benefitinformationtypeCount % recordsperpage > 0)
                                                                                            noOfPages += 1;
                                                                                    }
                                                                                    if(total > 0)
                                                                                    {
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=benefitinformationtypeCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getBenefitinformationtypeId()%>');"><%= info.getBenefitinformationtypeName() != null ? info.getBenefitinformationtypeName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getBenefitinformationtypeId()%>');"><%= info.getBenefittypeName() != null ? info.getBenefittypeName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getBenefitinformationtypeId()%>');"><%= benefitinformationtype.getStatusById(info.getStatus())%></td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getBenefitinformationtypeId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getBenefitinformationtypeId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
<%
                                                            }
