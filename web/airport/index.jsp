...
<html lang="en">
    ...
    <%
            ...
            int count = 0;
            int recordsperpage = airport.getCount();
            ...
            int showsizelist = airport.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <head>
        <meta charset="utf-8">
        <title><%= airport.getMainPath("title") != null ? airport.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/airport.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/airport/AirportAction.do" onsubmit="return false;">
        <html:hidden property="doAdd"/>
        <html:hidden property="doModify"/>
        <html:hidden property="doView"/>    
        <html:hidden property="airportId"/>
        <!-- Begin page -->
                                ...
                                <div class="body-background mt_0">
                                    <div class="row" id='ajax_cat'>
                                        ...
                                        <%
                                                                                ...
                                                                                if (recordsperpage > 0)
                                                                                {
                                                                                    noOfPages = airportCount / recordsperpage;
                                                                                    if (airportCount % recordsperpage > 0)
                                                                                        noOfPages += 1;
                                                                                }
                                                                                if(total > 0)
                                                                                {
                                        ...
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=airportCount%>)                                                
                                            </div>
...
%>
                                                            <tr class='hand_cursor'>
                                                                <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAirportId()%>');"><%= info.getAirportName() != null ? info.getAirportName() : ""%></td>
                                                                <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAirportId()%>');"><%= info.getShortname() != null ? info.getShortname() : ""%></td>
                                                                <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAirportId()%>');"><%= info.getCityName() != null ? info.getCityName() : ""%></td>
                                                                <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAirportId()%>');"><%= info.getCountryName() != null ? info.getCountryName() : ""%></td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getAirportId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getAirportId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
<%
                                                            }
