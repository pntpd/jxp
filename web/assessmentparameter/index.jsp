...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("ASSESSMENTPARAMETER_LIST") != null)
                assessmentParameter_list = (ArrayList) session.getAttribute("ASSESSMENTPARAMETER_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= assessmentParameter.getMainPath("title") != null ? assessmentParameter.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/assessmentparameter.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/assessmentparameter/AssessmentParameterAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="assessmentParameterId"/>
            <!-- Begin page -->
                                ...
                                <div class="col-md-12 col-xl-12">
                                    <div class="body-background mt_0">
                                        <div class="row" id='ajax_cat'>
                                            ...
                                            <%
                                                                                        if (nextCount == 0 && assessmentParameterCount > recordsperpage)
                                                                                        {
                                                                                            next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                                                                                            nextclose = ("</i></a></li>");
                                                                                        }
                                                                                        else if (nextCount > 0 && assessmentParameterCount >
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=assessmentParameterCount%>)                                                
                                                </div>
                                                        ...
                                                        <table id="tech-companies-1" class="table table-striped">
                                                            ...
                                                            <tbody id="sort_id">
                                                                ...
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAssessmentParameterId()%>');"><%= assessmentParameter.changeNum( info.getAssessmentParameterId(),3) %></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAssessmentParameterId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAssessmentParameterId()%>');"> <%= assessmentParameter.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getAssessmentParameterId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getAssessmentParameterId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                                                                            }
