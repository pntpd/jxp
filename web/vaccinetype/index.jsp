...
<%
        ...
        int count = 0;
        int recordsperpage = vaccinetype.getCount();
        ...
        int showsizelist = vaccinetype.getCountList("show_size_list");
        int CurrPageNo = 1;
...
<head>
    <meta charset="utf-8">
    <title><%= vaccinetype.getMainPath("title") != null ? vaccinetype.getMainPath("title") : "" %></title>
    ...
    <script type="text/javascript" src="../jsnew/vaccinetype.js"></script>
    <script type="text/javascript" src="../jsnew/validation.js"></script>
</head>
<body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/vaccinetype/VaccinetypeAction.do" onsubmit="return false;">
    <html:hidden property="doAdd"/>
    <html:hidden property="doModify"/>
    <html:hidden property="doView"/>    
    <html:hidden property="vaccinetypeId"/>
        <!-- Begin page -->
...
<%
                                                ...
                                                ((nextCount*recordsperpage) + recordsperpage))
                                            {
                                                prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                prevclose = ("</i></a></li>");
                                                last = vaccinetypeCount;
                                            }
                                            else
                                            {
                                                recordsperpage = vaccinetypeCount;
                                            }
                                        }
                                        else
                                        {
                                        ...
                                        if (recordsperpage > 0)
                                        {
                                            noOfPages = vaccinetypeCount / recordsperpage;
                                            if (vaccinetypeCount % recordsperpage > 0)
                                                noOfPages += 1;
                                        }
                                        if(total > 0)
                                        {
%>
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=vaccinetypeCount%>)                                                
                                            </div>
...
%>
                                                            <tr class="hand_cursor" href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getVaccinetypeId()%>');"><%= info.getName() != null ? info.getName() : "" %></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getVaccinetypeId()%>');"><%= vaccinetype.getStatusById(info.getStatus()) %></td>

                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getVaccinetypeId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getVaccinetypeId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
<%
                                                            }
