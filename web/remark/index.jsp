...
<%@page import="com.web.jxp.remark.RemarkInfo"%>
<%@taglib uri="/WEB-INF/struts-html-el.tld" prefix="html"%>
<%@taglib uri="/WEB-INF/fmt.tld" prefix="fmt"%>
<%@taglib uri="/WEB-INF/c.tld" prefix="c"%>
<jsp:useBean id="remark" class="com.web.jxp.remark.Remark" scope="page"/>
<!doctype html>
<html lang="en">
    <%
        try
    ...
    <%
            ...
            ArrayList remark_list = new ArrayList();
            int count = 0;
            int recordsperpage = remark.getCount();
            if(session.getAttribute("COUNT_LIST") != null)
                count = Integer.parseInt((String) session.getAttribute("COUNT_LIST"));
            if(session.getAttribute("REMARK_LIST") != null)
                remark_list = (ArrayList) session.getAttribute("REMARK_LIST");
            int pageSize = count / recordsperpage;
            if(count % recordsperpage > 0)
                pageSize = pageSize + 1;

            int total = remark_list.size();
            int showsizelist = remark.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <head>
        <meta charset="utf-8">
        <title><%= remark.getMainPath("title") != null ? remark.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/remark.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/remark/RemarkAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="remarkId"/>
            <!-- Begin page -->
...
<%
                                                ...
                                                if (recordsperpage > 0)
                                                {
                                                    noOfPages = remarkCount / recordsperpage;
                                                    if (remarkCount % recordsperpage > 0)
                                                        noOfPages += 1;
                                                }
                                                if(total > 0)
                                                {
%>
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=remarkCount%>)                                                
                                                </div>
...
<%
                                                                ...
                                                                {
                                                                    info = (RemarkInfo) remark_list.get(i);
                                                                    if (info != null) 
                                                                    {
                                                                        status = info.getStatus();
%>
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getRemarkId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getRemarkId()%>');"> <%= remark.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getRemarkId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getRemarkId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                            }
