...
<html lang="en">
    ...
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/position/PositionAction.do" onsubmit="return false;" styleClass="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="row head_title_area">
                            ...
                            <div class="col-md-12 col-xl-12 tab_head_area">
                                <ul class='nav nav-tabs nav-tabs-custom' id='tab_menu'>
                                    <c:choose>
                                        <c:when test="${positionForm.assessmentDetailId <= 0}">
                                                        ...
                                                        <table class="table table-striped mb-0">
                                                            <thead>
                                                                <tr>
                                                                    <th width="6%">Code</th>
                                                                    <th width="31%">Name</th>
                                                                    <th width="31%">Parameter</th>
                                                                    <th width="10%" class="text-center">Minimum Score</th>
                                                                    <th width="12%" class="text-center">Passing Required</th>
    ...
    <%
                                                                                                         if(assessment_list_size > 0)
                                                                                                         {
                                                                                                             ...
                                                                                                             {
                                                                                                                 info = (PositionInfo) assessment_list.get(i);
                                                                                                                 if(info != null)
                                                                                                                 {

    %>
                                                                <tr>
                                                                    <td> <%= position.changeNum( info.getAssessmentId(),3) %></td> 
                                                                    ...
                                                                    <% if(info.getPassingFlag() == 1) {%><td class="text-center"> <span class="check_mark"><i class="fa fa-check" aria-hidden="true"></i></span></td><% } else {%><td></td><% } %>
                                                                    <td class="action_column">                                                                    
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: addAssessmentForm('<%= info.getAssessmentDetailId()%>');" class=" mr_15"><img src="../assets/images/pencil.png"/></a><% }%>
                                                                        <span class="switch_bth">                                           
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=info.getAssessmentDetailId()%>" <% if( info.getStatus() == 1){%>checked<% } if(!editper.equals("Y")) {%> disabled="true"<% }%> onclick="javascript: deleteAssessmentForm('<%= info.getAssessmentDetailId()%>', '<%= info.getStatus()%>');"/>
                                                                            </div>                                                                        
                                                                        </span>
                                                                    </td>
                                                                </tr>    
