...
<html lang="en">
    ...
    <%
        ...
        int count = 0;
        int recordsperpage = checkpoint.getCount();
        ...
        int showsizelist = checkpoint.getCountList("show_size_list");
        int CurrPageNo = 1;
    ...
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/checkpoint/CheckPointAction.do" onsubmit="return false;">
                                        ...
                                        <div class="row" id='ajax_cat'>
                                            ...
                                            <%
                                                    if (nextCount == 0 && checkpointCount > recordsperpage) {
                                                        ...
                                                        prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                        prevclose = ("</i></a></li>");
                                                        last = checkpointCount;
                                                    } else {
                                                        recordsperpage = checkpointCount;
                                                    }
                                                } else {
                                                    recordsperpage = checkpointCount;
                                                }
                                                ...
                                                int noOfPages = 1;
                                                if (recordsperpage > 0) {
                                                    noOfPages = checkpointCount / recordsperpage;
                                                    if (checkpointCount % recordsperpage > 0) {
                                                        noOfPages += 1;
                                                    }
                                                }
                                                if (total > 0) {
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages%>, &nbsp;(<%= nextCount * recordsperpage + 1%> - <%= last > 0 ? last : nextCount * recordsperpage + recordsperpage%> record(s) of <%=checkpointCount%>)                                                
                                                </div>
                                                        ...
                                                        <table id="tech-companies-1" class="table table-striped">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th width="15%" >
                                                                        <span><b>Minimum Requirement</b></span>
                                                                    </th>
                                                            ...
                                                            <tbody id="sort_id">
                                                                ...
                                                                %>
                                                                <tr>
                                                                    <td class="hand_cursor" href="javascript: void(0);" onclick="showDetail('<%= info.getCheckpointId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href="javascript: void(0);" onclick="showDetail('<%= info.getCheckpointId()%>');"><%= info.getClientname() != null ? info.getClientname() : ""%></td>
                                                                    <td class="hand_cursor" href="javascript: void(0);" onclick="showDetail('<%= info.getCheckpointId()%>');"><%= info.getClientassetname() != null ? info.getClientassetname() : ""%></td>
                                                                    <td class="hand_cursor" href="javascript: void(0);" onclick="showDetail('<%= info.getCheckpointId()%>');"><%= info.getPositionname() != null ? info.getPositionname() +" / "+ info.getGrade() : ""%></td>
                                                                    <% if( info.getFlag() == 1) {%><td class="primary_active text-center"><i class="fa fa-check" aria-hidden="true"></i></td><% } else {%><td></td><% } %>
                                                                    <td class="action_column">
                                                                        <% if (editper.equals("Y") && status == 1) {%><a href="javascript: modifyForm('<%= info.getCheckpointId()%>');" class="mr_15"><img src="../assets/images/pencil.png"></a><% } else {%><a href='javascript:;' ><span style='width: 35px;'>&nbsp;</span></a><%}%>
                                                                        <span class="switch_bth">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=i%>" <%if (status == 1) {%>checked <% }
                                                                                    if (!editper.equals("Y")) {%> disabled="true"<% }%> onclick="javascript: deleteForm('<%= info.getCheckpointId()%>', '<%=status%>', '<%=i%>');" />
                                                                            </div>
                                                                        </span>
                                                                    </td>																	
                                                                </tr>
