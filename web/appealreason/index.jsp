...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("APPEALREASON_LIST") != null)
                appealreason_list = (ArrayList) session.getAttribute("APPEALREASON_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= appealreason.getMainPath("title") != null ? appealreason.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/appealreason.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/appealreason/AppealreasonAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="appealreasonId"/>
            <!-- Begin page -->
...
%>
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=appealreasonCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor'>
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAppealreasonId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAppealreasonId()%>');"> <%= appealreason.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getAppealreasonId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getAppealreasonId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                            }
