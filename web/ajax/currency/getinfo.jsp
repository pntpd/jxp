...
<%@page import="com.web.jxp.currency.CurrencyInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="currency" class="com.web.jxp.currency.Currency" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = currency.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = currency.deleteCurrency(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = currency.getCount();
                ArrayList currency_list = currency.getCurrencyByName(search, n1, recordsperpage);
                int count = 0;
                if (currency_list.size() > 0) {
                    CurrencyInfo cinfo = (CurrencyInfo) currency_list.get(currency_list.size() - 1);
                    count = cinfo.getCurrencyId();
                    currency_list.remove(currency_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("CURRENCY_LIST", currency_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int currencyCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = currency_list.size();
                ...
                if (n1 == 0 && currencyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && currencyCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = currencyCount / recordsperpage;
                    if (currencyCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + currencyCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (CurrencyInfo) currency_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCurrencyId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCurrencyId()+"');\">" + currency.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getCurrencyId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getCurrencyId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + currency.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    currency_list = currency.getCurrencyByName(search, n, recordsperpage);
                    if (currency_list.size() > 0) {
                        CurrencyInfo cinfo = (CurrencyInfo) currency_list.get(currency_list.size() - 1);
                        count = cinfo.getCurrencyId();
                        currency_list.remove(currency_list.size() - 1);
                ...
                if (n == 0 && currencyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && currencyCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
