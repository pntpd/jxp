...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = assessment.getCountList("show_size_list");
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Code</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Mode</b></span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Parameter</b> </span>");
            ...
            String parameterIds = request.getParameter("parameterId") != null && !request.getParameter("parameterId").equals("") ? vobj.replaceint(request.getParameter("parameterId")) : "-1";
            String questionIds = request.getParameter("questionId") != null && !request.getParameter("questionId").equals("") ? vobj.replaceint(request.getParameter("questionId")) : "-1";
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = assessment.deleteAssessment(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = assessment.getCount();
                ArrayList assessment_list = assessment.getAssessmentByName(search, parameterId, questionId, n1, recordsperpage);
                int count = 0;
                if (assessment_list.size() > 0) {
                    AssessmentInfo cinfo = (AssessmentInfo) assessment_list.get(assessment_list.size() - 1);
                    count = cinfo.getAssessmentId();
                    assessment_list.remove(assessment_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("ASSESSMENT_LIST", assessment_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int assessmentCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = assessment_list.size();
                ...
                if (n1 == 0 && assessmentCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && assessmentCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = assessmentCount;
                } else {
                    recordsperpage = assessmentCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = assessmentCount / recordsperpage;
                    if (assessmentCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + assessmentCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (AssessmentInfo) assessment_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentId()+"');\">" + (assessment.changeNum(info.getAssessmentId(), 3)) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentId()+"');\">" + (info.getMode() != null ? info.getMode() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentId()+"');\">" + (info.getParaName() != null ? info.getParaName() : "") + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getAssessmentId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getAssessmentId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + assessment.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    assessment_list = assessment.getAssessmentByName(search, parameterId,questionId, n, recordsperpage);
                    if (assessment_list.size() > 0) {
                        AssessmentInfo cinfo = (AssessmentInfo) assessment_list.get(assessment_list.size() - 1);
                        count = cinfo.getAssessmentId();
                        assessment_list.remove(assessment_list.size() - 1);
                ...
                if (n == 0 && assessmentCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && assessmentCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
