...
<%@page import="com.web.jxp.onboardingkit.OnboardingkitInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="onboardingkit" class="com.web.jxp.onboardingkit.Onboardingkit" scope="page"/>
...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = onboardingkit.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Kit Name</b> </span>");            
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = onboardingkit.deleteOnboardingkit(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = onboardingkit.getCount();
                ArrayList onboardingkit_list = onboardingkit.getOnboardingkitByName(search, n1, recordsperpage);
                int count = 0;
                if (onboardingkit_list.size() > 0) {
                    OnboardingkitInfo cinfo = (OnboardingkitInfo) onboardingkit_list.get(onboardingkit_list.size() - 1);
                    count = cinfo.getOnboardingkitId();
                    onboardingkit_list.remove(onboardingkit_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("ONBOARDINGKIT_LIST", onboardingkit_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int onboardingkitCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = onboardingkit_list.size();
                ...
                if (n1 == 0 && onboardingkitCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && onboardingkitCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = onboardingkitCount;
                } else {
                    recordsperpage = onboardingkitCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = onboardingkitCount / recordsperpage;
                    if (onboardingkitCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + onboardingkitCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (OnboardingkitInfo) onboardingkit_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getOnboardingkitId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getOnboardingkitId()+"');\">" + onboardingkit.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getOnboardingkitId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getOnboardingkitId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + onboardingkit.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = onboardingkit.getCount();
                ArrayList onboardingkit_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    onboardingkit_list = onboardingkit.getOnboardingkitByName(search, n, recordsperpage);
                    if (onboardingkit_list.size() > 0) {
                        OnboardingkitInfo cinfo = (OnboardingkitInfo) onboardingkit_list.get(onboardingkit_list.size() - 1);
                        count = cinfo.getOnboardingkitId();
                        onboardingkit_list.remove(onboardingkit_list.size() - 1);
                ...
                if (n == 0 && onboardingkitCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && onboardingkitCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
