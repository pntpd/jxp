...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = doccheck.getCountList("show_size_list");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Assets</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Document</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = doccheck.deleteDoccheck(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = doccheck.getCount();
                ArrayList doccheck_list = doccheck.getDoccheckByName(search, n1, recordsperpage,clientIndex, assetIndex,allclient,  per,  cids,  assetids );
                int count = 0;
                if (doccheck_list.size() > 0) {
                    DoccheckInfo cinfo = (DoccheckInfo) doccheck_list.get(doccheck_list.size() - 1);
                    count = cinfo.getDoccheckId();
                    doccheck_list.remove(doccheck_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("DOCCHECK_LIST", doccheck_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int doccheckCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = doccheck_list.size();
                ...
                if (n1 == 0 && doccheckCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && doccheckCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = doccheckCount / recordsperpage;
                    if (doccheckCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + doccheckCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (DoccheckInfo) doccheck_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDoccheckId()+"');\">" + (info.getClientName() != null ? info.getClientName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDoccheckId()+"');\">" + (info.getAssetName() != null ? info.getAssetName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDoccheckId()+"');\">" + (info.getDoccheckName() != null ? info.getDoccheckName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDoccheckId()+"');\">" + (doccheck.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getDoccheckId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getDoccheckId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + doccheck.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = doccheck.getCount();
                ArrayList doccheck_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    doccheck_list = doccheck.getDoccheckByName(search, n, recordsperpage, clientIndex, assetIndex, allclient, per, cids, assetids);
                    if (doccheck_list.size() > 0) {
                        DoccheckInfo cinfo = (DoccheckInfo) doccheck_list.get(doccheck_list.size() - 1);
                        count = cinfo.getDoccheckId();
                        doccheck_list.remove(doccheck_list.size() - 1);
                ...
                if (n == 0 && doccheckCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && doccheckCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
