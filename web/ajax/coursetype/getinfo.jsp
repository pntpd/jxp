    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = coursetype.deleteCourseType(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList coursetype_list = coursetype.getCourseTypeByName(search, n1, recordsperpage);
                int count = 0;
                if (coursetype_list.size() > 0) {
                    CourseTypeInfo cinfo = (CourseTypeInfo) coursetype_list.get(coursetype_list.size() - 1);
                    count = cinfo.getCoursetypeId();
                    coursetype_list.remove(coursetype_list.size() - 1);
                }
                ...
                if (n1 == 0 && coursetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && coursetypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + coursetypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + coursetype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    coursetype_list = coursetype.getCourseTypeByName(search, n, recordsperpage);
                    if (coursetype_list.size() > 0) {
                        CourseTypeInfo cinfo = (CourseTypeInfo) coursetype_list.get(coursetype_list.size() - 1);
                        count = cinfo.getCoursetypeId();
                        coursetype_list.remove(coursetype_list.size() - 1);
                ...
                if (n == 0 && coursetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && coursetypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
