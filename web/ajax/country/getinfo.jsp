...
<%@page import="com.web.jxp.country.CountryInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="country" class="com.web.jxp.country.Country" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = country.getCountList("show_size_list");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Nationalty</b> </span></th>");
            sb.append("<th><span><b>ISD Code</b> </span></th>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = country.deleteCountry(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = country.getCount();
                ArrayList country_list = country.getCountryByName(search, n1, recordsperpage);
                int count = 0;
                if (country_list.size() > 0) {
                    CountryInfo cinfo = (CountryInfo) country_list.get(country_list.size() - 1);
                    count = cinfo.getCountryId();
                    country_list.remove(country_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("COUNTRY_LIST", country_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int countryCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = country_list.size();
                ...
                if (n1 == 0 && countryCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && countryCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = countryCount / recordsperpage;
                    if (countryCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + countryCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (CountryInfo) country_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCountryId() +"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCountryId() +"');\">" + (info.getNationality() != null ? info.getNationality() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCountryId() +"');\">" + (info.getCode() != null ? info.getCode() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCountryId() +"');\">" + country.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getCountryId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getCountryId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + country.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    country_list = country.getCountryByName(search, n, recordsperpage);
                    if (country_list.size() > 0) {
                        CountryInfo cinfo = (CountryInfo) country_list.get(country_list.size() - 1);
                        count = cinfo.getCountryId();
                        country_list.remove(country_list.size() - 1);
                ...
                if (n == 0 && countryCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && countryCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
