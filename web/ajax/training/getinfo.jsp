...
<%@page import="com.web.jxp.training.TrainingInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="training" class="com.web.jxp.training.Training" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = training.getCountList("show_size_list");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Priority</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = training.deleteTraining(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = training.getCount();
                ArrayList training_list = training.getTrainingByName(search, n1, recordsperpage,clientIndex, assetIndex,allclient, per, cids, assetids);
                int count = 0;
                if (training_list.size() > 0) {
                    TrainingInfo cinfo = (TrainingInfo) training_list.get(training_list.size() - 1);
                    count = cinfo.getTrainingId();
                    training_list.remove(training_list.size() - 1);
                }
                ...
                if (n1 == 0 && trainingCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && trainingCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = trainingCount / recordsperpage;
                    if (trainingCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + trainingCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (TrainingInfo) training_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTrainingId()+"');\">" + (info.getClientName()!= null ? info.getClientName(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTrainingId()+"');\">" + (info.getAssetName()!= null ? info.getAssetName(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTrainingId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTrainingId()+"');\">" + training.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getTrainingId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getTrainingId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + training.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = training.getCount();
                ArrayList training_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    training_list = training.getTrainingByName(search, n, recordsperpage,clientIndex, assetIndex, allclient, per, cids, assetids);
                    if (training_list.size() > 0) {
                        TrainingInfo cinfo = (TrainingInfo) training_list.get(training_list.size() - 1);
                        count = cinfo.getTrainingId();
                        training_list.remove(training_list.size() - 1);
                ...
                if (n == 0 && trainingCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && trainingCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
