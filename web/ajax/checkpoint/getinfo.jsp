...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='20%'><span><b>Checkpoint Name</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='15%'><span><b>Minimum Requirement</b> </span></th>");
            sb.append("<th width='15%' class='text-right'><span><b>Actions</b></span></th>");
            sb.append("</tr>");
            sb.append("</thead>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = checkpoint.deleteCheckPoint(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList checkpoint_list = checkpoint.getCheckPointByName(search, n1, recordsperpage);
                int count = 0;
                if (checkpoint_list.size() > 0) {
                    CheckPointInfo cinfo = (CheckPointInfo) checkpoint_list.get(checkpoint_list.size() - 1);
                    count = cinfo.getCheckpointId();
                    checkpoint_list.remove(checkpoint_list.size() - 1);
                }
                ...
                if (total > 0) {
                    ...
                    str.append("<td>" + checkpoint.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null)
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    checkpoint_list = checkpoint.getCheckPointByName(search, n, recordsperpage);
                    if (checkpoint_list.size() > 0) {
                        CheckPointInfo cinfo = (CheckPointInfo) checkpoint_list.get(checkpoint_list.size() - 1);
                        count = cinfo.getCheckpointId();
                        checkpoint_list.remove(checkpoint_list.size() - 1);
