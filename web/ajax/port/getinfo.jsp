...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = port.getCountList("show_size_list");
            ...
            sb.append("</th>");
             sb.append("<th><span><b>Country Name</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = port.deletePort(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = port.getCount();
                ArrayList port_list = port.getPortByName(search, n1, recordsperpage);
                int count = 0;
                if (port_list.size() > 0) {
                    PortInfo cinfo = (PortInfo) port_list.get(port_list.size() - 1);
                    count = cinfo.getPortId();
                    port_list.remove(port_list.size() - 1);
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = portCount / recordsperpage;
                    if (portCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + portCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (PortInfo) port_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getPortId() + "');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getPortId() + "');\">" + (info.getCountryName() != null ? info.getCountryName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getPortId() + "');\">" + port.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getPortId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getPortId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + port.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    port_list = port.getPortByName(search, n, recordsperpage);
                    if (port_list.size() > 0) {
                        PortInfo cinfo = (PortInfo) port_list.get(port_list.size() - 1);
                        count = cinfo.getPortId();
                        port_list.remove(port_list.size() - 1);
                ...
                if (n == 0 && portCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && portCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
