...
<%@page import="com.web.jxp.vaccine.VaccineInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="vaccine" class="com.web.jxp.vaccine.Vaccine" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = vaccine.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = vaccine.deleteVaccine(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = vaccine.getCount();
                ArrayList vaccine_list = vaccine.getVaccineByName(search, n1, recordsperpage);
                int count = 0;
                if (vaccine_list.size() > 0) {
                    VaccineInfo cinfo = (VaccineInfo) vaccine_list.get(vaccine_list.size() - 1);
                    count = cinfo.getVaccineId();
                    vaccine_list.remove(vaccine_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("VACCINE_LIST", vaccine_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int vaccineCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = vaccine_list.size();
                ...
                if (n1 == 0 && vaccineCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && vaccineCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = vaccineCount / recordsperpage;
                    if (vaccineCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + vaccineCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (VaccineInfo) vaccine_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getVaccineId() + "');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getVaccineId() + "');\">" + vaccine.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getVaccineId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getVaccineId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + vaccine.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = vaccine.getCount();
                ArrayList vaccine_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    vaccine_list = vaccine.getVaccineByName(search, n, recordsperpage);
                    if (vaccine_list.size() > 0) {
                        VaccineInfo cinfo = (VaccineInfo) vaccine_list.get(vaccine_list.size() - 1);
                        count = cinfo.getVaccineId();
                        vaccine_list.remove(vaccine_list.size() - 1);
                ...
                if (n == 0 && vaccineCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && vaccineCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
