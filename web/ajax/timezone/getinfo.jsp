...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Time Zone</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = timezone.deleteTimezone(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList timezone_list = timezone.getTimezoneByName(search, n1, recordsperpage);
                int count = 0;
                if (timezone_list.size() > 0) {
                    TimezoneInfo cinfo = (TimezoneInfo) timezone_list.get(timezone_list.size() - 1);
                    count = cinfo.getTimezoneId();
                    timezone_list.remove(timezone_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("TIMEZONE_LIST", timezone_list);
                ...
                if (n1 == 0 && timezoneCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && timezoneCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + timezoneCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) 
                    {
                        ...
                        if (info != null) 
                        {
                            ...
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getTimezoneId() + "');\">" + (info.getTimezoneName() != null ? info.getTimezoneName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getTimezoneId() + "');\">" + (info.getCountryName() != null ? info.getCountryName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getTimezoneId() + "');\">" + (info.getTimezoneCode() != null ? info.getTimezoneCode() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getTimezoneId() + "');\">" + (timezone.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getTimezoneId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getTimezoneId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + timezone.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    timezone_list = timezone.getTimezoneByName(search, n, recordsperpage);
                    if (timezone_list.size() > 0) {
                        TimezoneInfo cinfo = (TimezoneInfo) timezone_list.get(timezone_list.size() - 1);
                        count = cinfo.getTimezoneId();
                        timezone_list.remove(timezone_list.size() - 1);
                ...
                if (n == 0 && timezoneCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && timezoneCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
