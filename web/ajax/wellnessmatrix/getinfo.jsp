...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            String iplocal = wellnessmatrix.getLocalIp();
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = wellnessmatrix.deleteWellnessmatrix(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList wellnessmatrix_list = wellnessmatrix.getWellnessmatrixByName(search, n1, recordsperpage, allclient, per, cids, assetids, clientIdIndex, assetIdIndex);
                int count = 0;
                if (wellnessmatrix_list.size() > 0) {
                    WellnessmatrixInfo cinfo = (WellnessmatrixInfo) wellnessmatrix_list.get(wellnessmatrix_list.size() - 1);
                ...
                if (total > 0) 
                {
                    ...
                    str.append("<td>" + wellnessmatrix.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    wellnessmatrix_list = wellnessmatrix.getWellnessmatrixByName(search, n, recordsperpage, allclient, per, cids, assetids, clientIdIndex, assetIdIndex);
                    if (wellnessmatrix_list.size() > 0) {
                        WellnessmatrixInfo cinfo = (WellnessmatrixInfo) wellnessmatrix_list.get(wellnessmatrix_list.size() - 1);
                        count = cinfo.getClientId();
                        wellnessmatrix_list.remove(wellnessmatrix_list.size() - 1);
