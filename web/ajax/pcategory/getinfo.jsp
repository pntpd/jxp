...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = pcategory.getCountList("show_size_list");
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='45%'><span><b>Category</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='14%'><span><b>Department</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='12%' class='text-center'><span><b>Questions</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th class='text-right' width='14%'><span><b>Actions</b></span></th>");
            sb.append("</tr>");
            sb.append("</thead>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = pcategory.deletePcategory(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = pcategory.getCount();
                ArrayList pcategory_list = pcategory.getPcategoryByName(search, pdeptIdIndex, assettypeIdIndex, n1, recordsperpage);
                int count = 0;
                if (pcategory_list.size() > 0) {
                    PcategoryInfo cinfo = (PcategoryInfo) pcategory_list.get(pcategory_list.size() - 1);
                    count = cinfo.getPcategoryId();
                    pcategory_list.remove(pcategory_list.size() - 1);
                }
                ...
                if (n1 == 0 && pcategoryCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && pcategoryCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = pcategoryCount / recordsperpage;
                    if (pcategoryCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + pcategoryCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + pcategory.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    pcategory_list = pcategory.getPcategoryByName(search, pdeptIdIndex, assettypeIdIndex, n, recordsperpage); 
                    if (pcategory_list.size() > 0) {
                        PcategoryInfo cinfo = (PcategoryInfo) pcategory_list.get(pcategory_list.size() - 1);
                        count = cinfo.getPcategoryId();
                        pcategory_list.remove(pcategory_list.size() - 1);
                ...
                if (n == 0 && pcategoryCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && pcategoryCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
