...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = pcode.getCountList("show_size_list");
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='37%'><span><b>Role Competency</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='9%'><span><b>Code</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='15%'><span><b>Asset Type</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='15%'><span><b>Department</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='11%' class='text-center'><span><b>Category</b></span> </th>");
            sb.append("<th class='text-right' width='13%'><span><b>Actions</b></span></th>");
            sb.append("</tr>");
            sb.append("</thead>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                String status_s = request.getParameter("status") != null && !request.getParameter("status").equals("") ? vobj.replaceint(request.getParameter("status")) : "0";
                String assettypeIdIndex_s = request.getParameter("assettypeIdIndex") != null && !request.getParameter("assettypeIdIndex").equals("") ? vobj.replaceint(request.getParameter("assettypeIdIndex")) : "0";
                String pdeptIdIndex_s = request.getParameter("pdeptIdIndex") != null && !request.getParameter("pdeptIdIndex").equals("") ? vobj.replaceint(request.getParameter("pdeptIdIndex")) : "0";
                int id = Integer.parseInt(delete_ids);
                int statusval = Integer.parseInt(status_s);
                int assettypeIdIndex = Integer.parseInt(assettypeIdIndex_s);
                int pdeptIdIndex = Integer.parseInt(pdeptIdIndex_s);
                int result = pcode.deletePcode(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = pcode.getCount();
                ArrayList pcode_list = pcode.getPcodeByName(search, pdeptIdIndex, assettypeIdIndex, n1, recordsperpage);
                int count = 0;
                if (pcode_list.size() > 0) {
                    PcodeInfo cinfo = (PcodeInfo) pcode_list.get(pcode_list.size() - 1);
                    count = cinfo.getPcodeId();
                    pcode_list.remove(pcode_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("PCODE_LIST", pcode_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int pcodeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = pcode_list.size();
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = pcodeCount / recordsperpage;
                    if (pcodeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + pcodeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (PcodeInfo) pcode_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr>");
                            str.append("<td>" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td>" + (info.getCode() != null ? info.getCode() : "") + "</td>");
                            str.append("<td>" + (info.getAssettypeName() != null ? info.getAssettypeName() : "") + "</td>");
                            str.append("<td>" + (info.getDeptName() != null ? info.getDeptName() : "") + "</td>");
                            str.append("<td  class='assets_list text-center hand_cursor'><a href=\"javascript: showDetail('"+info.getPcodeId()+"');\">" + (info.getQcount()) + "</a></td>");
                            str.append("<td class='action_column'>");
                            str.append("<a class='mr_15' href=\"javascript: showDetail('"+info.getPcodeId()+"');\"><img src='../assets/images/plus.png' /></a>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getPcodeId() + "');\" class='mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getPcodeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");                            
                            str.append("</td>");
                            str.append("</tr>");
                    ...
                    str.append("<td>" + pcode.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = pcode.getCount();
                ArrayList pcode_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    pcode_list = pcode.getPcodeByName(search, pdeptIdIndex, assettypeIdIndex, n, recordsperpage); 
                    if (pcode_list.size() > 0) {
                        PcodeInfo cinfo = (PcodeInfo) pcode_list.get(pcode_list.size() - 1);
                        count = cinfo.getPcodeId();
                        pcode_list.remove(pcode_list.size() - 1);
                ...
                if (n == 0 && pcodeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && pcodeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
