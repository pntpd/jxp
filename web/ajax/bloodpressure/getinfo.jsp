...
<%@page import="com.web.jxp.bloodpressure.BloodpressureInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="bloodpressure" class="com.web.jxp.bloodpressure.Bloodpressure" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = bloodpressure.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = bloodpressure.deleteBloodpressure(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = bloodpressure.getCount();
                ArrayList bloodpressure_list = bloodpressure.getBloodpressureByName(search, n1, recordsperpage);
                int count = 0;
                if (bloodpressure_list.size() > 0) {
                    BloodpressureInfo cinfo = (BloodpressureInfo) bloodpressure_list.get(bloodpressure_list.size() - 1);
                    count = cinfo.getBloodpressureId();
                    bloodpressure_list.remove(bloodpressure_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("BLOODPRESSURE_LIST", bloodpressure_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int bloodpressureCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = bloodpressure_list.size();
                ...
                if (n1 == 0 && bloodpressureCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && bloodpressureCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = bloodpressureCount;
                } else {
                    recordsperpage = bloodpressureCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = bloodpressureCount / recordsperpage;
                    if (bloodpressureCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + bloodpressureCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (BloodpressureInfo) bloodpressure_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBloodpressureId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBloodpressureId()+"');\">" + bloodpressure.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getBloodpressureId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getBloodpressureId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + bloodpressure.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null)
            {
                ...
                int recordsperpage = bloodpressure.getCount();
                ArrayList bloodpressure_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    bloodpressure_list = bloodpressure.getBloodpressureByName(search, n, recordsperpage);
                    if (bloodpressure_list.size() > 0) {
                        BloodpressureInfo cinfo = (BloodpressureInfo) bloodpressure_list.get(bloodpressure_list.size() - 1);
                        count = cinfo.getBloodpressureId();
                        bloodpressure_list.remove(bloodpressure_list.size() - 1);
                ...
                if (n == 0 && bloodpressureCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && bloodpressureCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
