...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = contract.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = contract.deleteContract(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = contract.getCount();
                ArrayList contract_list = contract.getContractByName(search, n1, recordsperpage,allclient, per, cids, assetids);
                int count = 0;
                if (contract_list.size() > 0) {
                    ContractInfo cinfo = (ContractInfo) contract_list.get(contract_list.size() - 1);
                    count = cinfo.getContractId();
                    contract_list.remove(contract_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("CONTRACT_LIST", contract_list);
                ...
                if (n1 == 0 && contractCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && contractCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    {
                        info = (ContractInfo) contract_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getContractId()+"');\">" + (info.getClientname()!= null ? info.getClientname(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getContractId()+"');\">" + (info.getAssetName()!= null ? info.getAssetName(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getContractId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getContractId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getContractId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + contract.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    contract_list = contract.getContractByName(search, n, recordsperpage,allclient, per, cids, assetids);
                    if (contract_list.size() > 0) {
                        ContractInfo cinfo = (ContractInfo) contract_list.get(contract_list.size() - 1);
                        count = cinfo.getContractId();
                        contract_list.remove(contract_list.size() - 1);
