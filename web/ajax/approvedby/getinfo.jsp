...
<%@page import="com.web.jxp.approvedby.ApprovedbyInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="approvedby" class="com.web.jxp.approvedby.Approvedby" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = approvedby.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = approvedby.deleteApprovedby(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = approvedby.getCount();
                ArrayList approvedby_list = approvedby.getApprovedbyByName(search, n1, recordsperpage);
                int count = 0;
                if (approvedby_list.size() > 0) {
                    ApprovedbyInfo cinfo = (ApprovedbyInfo) approvedby_list.get(approvedby_list.size() - 1);
                    count = cinfo.getApprovedbyId();
                    approvedby_list.remove(approvedby_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("APPROVEDBY_LIST", approvedby_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int approvedbyCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = approvedby_list.size();
                ...
                if (n1 == 0 && approvedbyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && approvedbyCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = approvedbyCount;
                } else {
                    recordsperpage = approvedbyCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = approvedbyCount / recordsperpage;
                    if (approvedbyCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + approvedbyCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (ApprovedbyInfo) approvedby_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getApprovedbyId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getApprovedbyId()+"');\">" + approvedby.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getApprovedbyId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getApprovedbyId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + approvedby.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            } else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = approvedby.getCount();
                ArrayList approvedby_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    approvedby_list = approvedby.getApprovedbyByName(search, n, recordsperpage);
                    if (approvedby_list.size() > 0) {
                        ApprovedbyInfo cinfo = (ApprovedbyInfo) approvedby_list.get(approvedby_list.size() - 1);
                        count = cinfo.getApprovedbyId();
                        approvedby_list.remove(approvedby_list.size() - 1);
                ...
                if (n == 0 && approvedbyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && approvedbyCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
