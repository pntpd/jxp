    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = vaccinetype.deleteVaccinetype(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList vaccinetype_list = vaccinetype.getVaccinetypeByName(search, n1, recordsperpage);
                int count = 0;
                if (vaccinetype_list.size() > 0) {
                    VaccinetypeInfo cinfo = (VaccinetypeInfo) vaccinetype_list.get(vaccinetype_list.size() - 1);
                    count = cinfo.getVaccinetypeId();
                    vaccinetype_list.remove(vaccinetype_list.size() - 1);
                }
                ...
                if (n1 == 0 && vaccinetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && vaccinetypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + vaccinetypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) 
                    {
                        ...
                        if (info != null)
                        {
                            ...
                            str.append("</div>");
                            str.append("</span>");
                            str.append("<a href=\"javascript: showDetail('" + info.getVaccinetypeId() + "');\" class='view_mode float-end mr_15'><i class='ion ion-ios-search'></i></a>");
                    ...
                    str.append("<td>" + vaccinetype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    vaccinetype_list = vaccinetype.getVaccinetypeByName(search, n, recordsperpage);
                    if (vaccinetype_list.size() > 0) {
                        VaccinetypeInfo cinfo = (VaccinetypeInfo) vaccinetype_list.get(vaccinetype_list.size() - 1);
                        count = cinfo.getVaccinetypeId();
                        vaccinetype_list.remove(vaccinetype_list.size() - 1);
                ...
                if (n == 0 && vaccinetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && vaccinetypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
