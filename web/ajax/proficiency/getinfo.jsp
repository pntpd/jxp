...
<%@page import="com.web.jxp.proficiency.ProficiencyInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="proficiency" class="com.web.jxp.proficiency.Proficiency" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = proficiency.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = proficiency.deleteProficiency(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = proficiency.getCount();
                ArrayList proficiency_list = proficiency.getProficiencyByName(search, n1, recordsperpage);
                int count = 0;
                if (proficiency_list.size() > 0) {
                    ProficiencyInfo cinfo = (ProficiencyInfo) proficiency_list.get(proficiency_list.size() - 1);
                    count = cinfo.getProficiencyId();
                    proficiency_list.remove(proficiency_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("PROFICIENCY_LIST", proficiency_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int proficiencyCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = proficiency_list.size();
                ...
                if (n1 == 0 && proficiencyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && proficiencyCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = proficiencyCount;
                } else {
                    recordsperpage = proficiencyCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = proficiencyCount / recordsperpage;
                    if (proficiencyCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + proficiencyCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (ProficiencyInfo) proficiency_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);' >");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getProficiencyId() + "');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getProficiencyId() + "');\">" + proficiency.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getProficiencyId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getProficiencyId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + proficiency.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = proficiency.getCount();
                ArrayList proficiency_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    proficiency_list = proficiency.getProficiencyByName(search, n, recordsperpage);
                    if (proficiency_list.size() > 0) {
                        ProficiencyInfo cinfo = (ProficiencyInfo) proficiency_list.get(proficiency_list.size() - 1);
                        count = cinfo.getProficiencyId();
                        proficiency_list.remove(proficiency_list.size() - 1);
                ...
                if (n == 0 && proficiencyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && proficiencyCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
