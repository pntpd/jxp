...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = documentissuedby.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>IssueAuthority</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Documents</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Countries</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = documentissuedby.deleteDocumentissuedby(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = documentissuedby.getCount();
                ArrayList documentissuedby_list = documentissuedby.getDocumentissuedbyByName(search, n1, recordsperpage);
                int count = 0;
                if (documentissuedby_list.size() > 0) {
                    DocumentissuedbyInfo cinfo = (DocumentissuedbyInfo) documentissuedby_list.get(documentissuedby_list.size() - 1);
                    count = cinfo.getDocumentissuedbyId();
                    documentissuedby_list.remove(documentissuedby_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("DOCUMENTISSUEDBY_LIST", documentissuedby_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int documentissuedbyCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = documentissuedby_list.size();
                ...
                if (n1 == 0 && documentissuedbyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && documentissuedbyCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = documentissuedbyCount;
                } else {
                    recordsperpage = documentissuedbyCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = documentissuedbyCount / recordsperpage;
                    if (documentissuedbyCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + documentissuedbyCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (DocumentissuedbyInfo) documentissuedby_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDocumentissuedbyId()+"');\">" + (info.getDocumentissuedbyName() != null ? info.getDocumentissuedbyName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDocumentissuedbyId()+"');\">" + (info.getDocumentName() != null ? info.getDocumentName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDocumentissuedbyId()+"');\">" + (info.getCountryName() != null ? info.getCountryName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getDocumentissuedbyId()+"');\">" + (documentissuedby.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getDocumentissuedbyId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getDocumentissuedbyId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + documentissuedby.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = documentissuedby.getCount();
                ArrayList documentissuedby_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    documentissuedby_list = documentissuedby.getDocumentissuedbyByName(search, n, recordsperpage);
                    if (documentissuedby_list.size() > 0) {
                        DocumentissuedbyInfo cinfo = (DocumentissuedbyInfo) documentissuedby_list.get(documentissuedby_list.size() - 1);
                        count = cinfo.getDocumentissuedbyId();
                        documentissuedby_list.remove(documentissuedby_list.size() - 1);
                ...
                if (n == 0 && documentissuedbyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && documentissuedbyCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
