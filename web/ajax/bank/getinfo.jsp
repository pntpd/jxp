...
<%@page import="com.web.jxp.bank.BankInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="bank" class="com.web.jxp.bank.Bank" scope="page"/>
...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = bank.getCountList("show_size_list");
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Bank Name</b> </span>");            
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Branch Name</b> </span>");            
            ...
            sb.append("</th>");
            sb.append("<th><span><b>A/C No</b> </span>");            
            ...
            sb.append("</th>");
            sb.append("<th><span><b>IFSC Code</b> </span>");            
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = bank.deleteBank(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = bank.getCount();
                ArrayList bank_list = bank.getBankByName(search, n1, recordsperpage);
                int count = 0;
                if (bank_list.size() > 0) {
                    BankInfo cinfo = (BankInfo) bank_list.get(bank_list.size() - 1);
                    count = cinfo.getBankId();
                    bank_list.remove(bank_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("PAYMENTMODE_LIST", bank_list);
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = bankCount / recordsperpage;
                    if (bankCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + bankCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (BankInfo) bank_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankId()+"');\">" + (info.getBranch() != null ? info.getBranch() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankId()+"');\">" + (info.getAccno() != null ? info.getAccno() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankId()+"');\">" + (info.getIfsc() != null ? info.getIfsc() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankId()+"');\">" + bank.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getBankId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getBankId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + bank.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    bank_list = bank.getBankByName(search, n, recordsperpage);
                    if (bank_list.size() > 0) {
                        BankInfo cinfo = (BankInfo) bank_list.get(bank_list.size() - 1);
                        count = cinfo.getBankId();
                        bank_list.remove(bank_list.size() - 1);
                ...
                if (n == 0 && bankCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && bankCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
