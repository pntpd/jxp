...
<%@page import="com.web.jxp.invoicetemplate.InvoicetemplateInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="invoicetemplate" class="com.web.jxp.invoicetemplate.Invoicetemplate" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = invoicetemplate.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Client Name</b> </span>");            
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Template</b> </span>");            
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = invoicetemplate.deleteInvoicetemplate(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = invoicetemplate.getCount();
                ArrayList invoicetemplate_list = invoicetemplate.getInvoicetemplateByName(search, n1, recordsperpage,allclient, per, cids, assetids);
                int count = 0;
                if (invoicetemplate_list.size() > 0) {
                    InvoicetemplateInfo cinfo = (InvoicetemplateInfo) invoicetemplate_list.get(invoicetemplate_list.size() - 1);
                    count = cinfo.getInvoicetemplateId();
                    invoicetemplate_list.remove(invoicetemplate_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("INVOICETEMPLATE_LIST", invoicetemplate_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int invoicetemplateCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = invoicetemplate_list.size();
                ...
                if (n1 == 0 && invoicetemplateCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && invoicetemplateCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = invoicetemplateCount;
                } else {
                    recordsperpage = invoicetemplateCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = invoicetemplateCount / recordsperpage;
                    if (invoicetemplateCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + invoicetemplateCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (InvoicetemplateInfo) invoicetemplate_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getInvoicetemplateId()+"');\">" + (info.getClientname()!= null ? info.getClientname(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getInvoicetemplateId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getInvoicetemplateId()+"');\">" + invoicetemplate.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getInvoicetemplateId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getInvoicetemplateId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + invoicetemplate.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = invoicetemplate.getCount();
                ArrayList invoicetemplate_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    invoicetemplate_list = invoicetemplate.getInvoicetemplateByName(search, n, recordsperpage,allclient,per,cids,assetids);
                    if (invoicetemplate_list.size() > 0) {
                        InvoicetemplateInfo cinfo = (InvoicetemplateInfo) invoicetemplate_list.get(invoicetemplate_list.size() - 1);
                        count = cinfo.getInvoicetemplateId();
                        invoicetemplate_list.remove(invoicetemplate_list.size() - 1);
                ...
                if (n == 0 && invoicetemplateCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && invoicetemplateCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
