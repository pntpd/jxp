...
<%@page import="com.web.jxp.coursename.CoursenameInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="coursename" class="com.web.jxp.coursename.Coursename" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = coursename.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='80%'><span><b>Name</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='10%'><span><b>Status</b> </span> </th>");
            sb.append("<th class='text-right' width='10%'><span><b>Actions</b></span></th>");
            sb.append("</tr>");
            sb.append("</thead>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = coursename.deleteCoursename(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = coursename.getCount();
                ArrayList coursename_list = coursename.getCoursenameByName(search, n1, recordsperpage);
                int count = 0;
                if (coursename_list.size() > 0) {
                    CoursenameInfo cinfo = (CoursenameInfo) coursename_list.get(coursename_list.size() - 1);
                    count = cinfo.getCoursenameId();
                    coursename_list.remove(coursename_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("COURSENAME_LIST", coursename_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int coursenameCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = coursename_list.size();
                ...
                if (n1 == 0 && coursenameCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && coursenameCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = coursenameCount;
                } else {
                    recordsperpage = coursenameCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = coursenameCount / recordsperpage;
                    if (coursenameCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + coursenameCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (CoursenameInfo) coursename_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCoursenameId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCoursenameId()+"');\">" + coursename.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getCoursenameId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getCoursenameId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + coursename.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = coursename.getCount();
                ArrayList coursename_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    coursename_list = coursename.getCoursenameByName(search, n, recordsperpage);
                    if (coursename_list.size() > 0) {
                        CoursenameInfo cinfo = (CoursenameInfo) coursename_list.get(coursename_list.size() - 1);
                        count = cinfo.getCoursenameId();
                        coursename_list.remove(coursename_list.size() - 1);
                ...
                if (n == 0 && coursenameCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && coursenameCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
