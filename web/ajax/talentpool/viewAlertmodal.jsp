    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("candidateId") != null) 
            {
                ...
                String names = request.getParameter("name") != null ? vobj.replacename(request.getParameter("name")) : "";
                String positions = request.getParameter("position") != null ? vobj.replacename(request.getParameter("position")) : "";
                String grades = request.getParameter("grade") != null ? vobj.replacename(request.getParameter("grade")) : "";
                ...
                if (candidateId > 0)
                {
                    ...
                    if (size > 0) 
                    {
                        sb.append("<h2>" + names + " | " + positions + " | " + grades + " - ALERTS</h2>");
                        sb.append("<div>");
                        sb.append("<select class='form-select' name = 'typeId' id='typeId' onchange=\"javascript: showExpiryDetails('"+candidateId+"', '"+typeId+"');\">");
                            sb.append("<option value='-1'>Select</option>");
                            sb.append("<option value='1'>Expired</option>");
                            sb.append("<option value='2'>Verify</option>");
                        sb.append("</select>");
                        sb.append("</div>");
...
//                                    }
                                    if (info.getType() == 1) 
                                    {
                                        sb.append("<td class='hand_cursor text-center'><a class='ver_act' href=\"../verification/VerificationAction.do?doView=yes&verificationId=" + candidateId + "&tb=" + info.getTabno() + "&alertId="+info.getAlertId()+"\"  target = '_blank'>Verify</a></td>");
                                    }
                                    sb.append("</tr>");
                                }
