...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            sb.append("</th>");
            sb.append("<th  class='text-center'><span><b>Sub-category</b></span></th>");
            sb.append("<th  class='text-center'><span><b>Courses</b></span></th>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = createtraining.deleteCreatetraining(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList createtraining_list = createtraining.getCreatetrainingByName(search, n1, recordsperpage);
                int count = 0;
                if (createtraining_list.size() > 0) {
                    CreatetrainingInfo cinfo = (CreatetrainingInfo) createtraining_list.get(createtraining_list.size() - 1);
                    count = cinfo.getCategoryId();
                    createtraining_list.remove(createtraining_list.size() - 1);
                }
                ...
                if (n1 == 0 && createtrainingCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && createtrainingCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + createtrainingCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) 
                    {
                        ...
                        if (info != null) 
                        {
                            ...
                            str.append("<td class='assets_list text-center'><a href=\"javascript: viewSubcategory('"+ info.getCategoryId()+"');\">"+  createtraining.changeNum( info.getSubcategorycount(),2)+"</a></td>");
                            str.append("<td class='assets_list text-center'><a href=\"javascript: viewCoursecat('"+info.getCategoryId()+"');\">"+  createtraining.changeNum( info.getCoursecount(),2)+"</a></td>");
                    ...
                    str.append("<td>" + createtraining.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            } else if (request.getParameter("deleteVal") == null)
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    createtraining_list = createtraining.getCreatetrainingByName(search, n, recordsperpage);
                    if (createtraining_list.size() > 0) {
                        CreatetrainingInfo cinfo = (CreatetrainingInfo) createtraining_list.get(createtraining_list.size() - 1);
                        count = cinfo.getCategoryId();
                        createtraining_list.remove(createtraining_list.size() - 1);
                ...
                if (n == 0 && createtrainingCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && createtrainingCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
