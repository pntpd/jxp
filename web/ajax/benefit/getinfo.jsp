...
<%@page import="com.web.jxp.benefit.BenefitInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="benefit" class="com.web.jxp.benefit.Benefit" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = benefit.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Benefit</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Benefit Types</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = benefit.deleteBenefit(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = benefit.getCount();
                ArrayList benefit_list = benefit.getBenefitByName(search, n1, recordsperpage);
                int count = 0;
                if (benefit_list.size() > 0) {
                    BenefitInfo cinfo = (BenefitInfo) benefit_list.get(benefit_list.size() - 1);
                    count = cinfo.getBenefitId();
                    benefit_list.remove(benefit_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("BENEFIT_LIST", benefit_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int benefitCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = benefit_list.size();
                ...
                if (n1 == 0 && benefitCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && benefitCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = benefitCount / recordsperpage;
                    if (benefitCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + benefitCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (BenefitInfo) benefit_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBenefitId()+"');\">" + (info.getBenefitName() != null ? info.getBenefitName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBenefitId()+"');\">" + (info.getBenefittypeName() != null ? info.getBenefittypeName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBenefitId()+"');\">" + (benefit.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getBenefitId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getBenefitId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + benefit.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = benefit.getCount();
                ArrayList benefit_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    benefit_list = benefit.getBenefitByName(search, n, recordsperpage);
                    if (benefit_list.size() > 0) {
                        BenefitInfo cinfo = (BenefitInfo) benefit_list.get(benefit_list.size() - 1);
                        count = cinfo.getBenefitId();
                        benefit_list.remove(benefit_list.size() - 1);
                ...
                if (n == 0 && benefitCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && benefitCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
