...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='14%'><span><b>Position</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='15%'><span><b>Rank</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='40%'><span><b>Minimum Qualification</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='10%' class='text-center'><span><b>Crew Rotation</b> </span></th>");
            sb.append("<th width='6%' class='text-center'><span><b>Actions</b></span></th>");
            sb.append("</tr>");
            sb.append("</thead>");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = position.deletePosition(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList position_list = position.getPositionByName(search, n1, recordsperpage);
                int count = 0;
                if (position_list.size() > 0) {
                    PositionInfo cinfo = (PositionInfo) position_list.get(position_list.size() - 1);
                    count = cinfo.getPositionId();
                    position_list.remove(position_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("POSITION_LIST", position_list);
                ...
                if (n1 == 0 && positionCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && positionCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + positionCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) {
                        ...
                        {
                            str.append("<tr>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick='showDetail(" + info.getPositionId() + ");'>" + (info.getPositiontitle() != null && !info.getPositiontitle().equals("") ? info.getPositiontitle() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick='showDetail(" + info.getPositionId() + ");'>" + (info.getGrade() != null && !info.getGrade().equals("") ? info.getGrade() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick='showDetail(" + info.getPositionId() + ");'>" + (info.getAsset() != null && !info.getAsset().equals("") ? info.getAsset() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick='showDetail(" + info.getPositionId() + ");'>" + (info.getMinqualification() != null && !info.getMinqualification().equals("") ? info.getMinqualification() : "") + "</td>");
                            str.append("<td class='assets_list text-center'>");
                            if (info.getRotation() != null && !info.getRotation().equals("")) {
                                str.append("<a href=\"javascript: showDetail('" + info.getPositionId() + "');\" >" + info.getRotation() + "</a>");
                            } else {
                                str.append("&nbsp;");
                            }
                            str.append("</td>");
                            ...
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getPositionId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + position.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    position_list = position.getPositionByName(search, n, recordsperpage);
                    if (position_list.size() > 0) {
                        PositionInfo cinfo = (PositionInfo) position_list.get(position_list.size() - 1);
                        count = cinfo.getPositionId();
                        position_list.remove(position_list.size() - 1);
                ...
                if (n == 0 && positionCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && positionCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
