...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            sb.append("</th>");
            sb.append("<th width='12%' class='text-center'>");
            ...
            if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    billingcycle_list = billingcycle.getBillingcycleByName(search, n, recordsperpage, allclient, per, cids, assetids, clientIdIndex, assetIdIndex);
                    if (billingcycle_list.size() > 0) {
                        BillingcycleInfo cinfo = (BillingcycleInfo) billingcycle_list.get(billingcycle_list.size() - 1);
                        count = cinfo.getClientId();
                        billingcycle_list.remove(billingcycle_list.size() - 1);
                    }
                } else {
                    billingcycle_list = billingcycle.getBillingcycleByName(search, n, recordsperpage, allclient, per, cids, assetids, clientIdIndex, assetIdIndex);
                    if (billingcycle_list.size() > 0) {
                ...
                if (n == 0 && billingcycleCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && billingcycleCount > ((n * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + billingcycleCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + billingcycle.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
