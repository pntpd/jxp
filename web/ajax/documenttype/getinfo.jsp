...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='20%'><span><b>Document Name</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th width='50%'><span><b>Module-Sub-Module</b> </span>");
            sb.append("</th>");
            sb.append("<th width='20%'><span><b>Format</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = documentType.deleteDocumentType(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList documentType_list = documentType.getDocumentTypeByName(search, n1, recordsperpage);
                int count = 0;
                if (documentType_list.size() > 0) {
                    DocumentTypeInfo cinfo = (DocumentTypeInfo) documentType_list.get(documentType_list.size() - 1);
                    count = cinfo.getDocumentTypeId();
                    documentType_list.remove(documentType_list.size() - 1);
                }
                ...
                if (n1 == 0 && documentTypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && documentTypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + documentTypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + documentType.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    documentType_list = documentType.getDocumentTypeByName(search, n, recordsperpage);
                    if (documentType_list.size() > 0) {
                        DocumentTypeInfo cinfo = (DocumentTypeInfo) documentType_list.get(documentType_list.size() - 1);
                        count = cinfo.getDocumentTypeId();
                        documentType_list.remove(documentType_list.size() - 1);
                ...
                if (n == 0 && documentTypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && documentTypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
