...
<%@page import="com.web.jxp.revision.RevisionInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="revision" class="com.web.jxp.revision.Revision" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = revision.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = revision.deleteRevision(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = revision.getCount();
                ArrayList revision_list = revision.getRevisionByName(search, n1, recordsperpage);
                int count = 0;
                if (revision_list.size() > 0) {
                    RevisionInfo cinfo = (RevisionInfo) revision_list.get(revision_list.size() - 1);
                    count = cinfo.getRevisionId();
                    revision_list.remove(revision_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("REVISION_LIST", revision_list);
                ...
                if (n1 == 0 && revisionCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && revisionCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = revisionCount / recordsperpage;
                    if (revisionCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + revisionCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (RevisionInfo) revision_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRevisionId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRevisionId()+"');\">" + revision.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getRevisionId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getRevisionId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + revision.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = revision.getCount();
                ArrayList revision_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    revision_list = revision.getRevisionByName(search, n, recordsperpage);
                    if (revision_list.size() > 0) {
                        RevisionInfo cinfo = (RevisionInfo) revision_list.get(revision_list.size() - 1);
                        count = cinfo.getRevisionId();
                        revision_list.remove(revision_list.size() - 1);
                ...
                if (n == 0 && revisionCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && revisionCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
