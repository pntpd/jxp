...
<%@page import="com.web.jxp.hoursofwork.HoursOfWorkInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="hoursofwork" class="com.web.jxp.hoursofwork.HoursOfWork" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = hoursofwork.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>No Of Hours</b> </span></th>");            
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = hoursofwork.deleteHoursOfWork(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = hoursofwork.getCount();
                ArrayList hoursofwork_list = hoursofwork.getHoursOfWorkByName(search, n1, recordsperpage);
                int count = 0;
                if (hoursofwork_list.size() > 0) {
                    HoursOfWorkInfo cinfo = (HoursOfWorkInfo) hoursofwork_list.get(hoursofwork_list.size() - 1);
                    count = cinfo.getHoursofworkId();
                    hoursofwork_list.remove(hoursofwork_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("HOURSOFWORK_LIST", hoursofwork_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int hoursofworkCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = hoursofwork_list.size();
                ...
                if (n1 == 0 && hoursofworkCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && hoursofworkCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = hoursofworkCount;
                } else {
                    recordsperpage = hoursofworkCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = hoursofworkCount / recordsperpage;
                    if (hoursofworkCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + hoursofworkCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (HoursOfWorkInfo) hoursofwork_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getHoursofworkId()+"');\">" + (info.getNoOfHours()) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getHoursofworkId()+"');\">" + hoursofwork.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getHoursofworkId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getHoursofworkId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + hoursofwork.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = hoursofwork.getCount();
                ArrayList hoursofwork_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    hoursofwork_list = hoursofwork.getHoursOfWorkByName(search, n, recordsperpage);
                    if (hoursofwork_list.size() > 0) {
                        HoursOfWorkInfo cinfo = (HoursOfWorkInfo) hoursofwork_list.get(hoursofwork_list.size() - 1);
                        count = cinfo.getHoursofworkId();
                        hoursofwork_list.remove(hoursofwork_list.size() - 1);
                ...
                if (n == 0 && hoursofworkCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && hoursofworkCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
