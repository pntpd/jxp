    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = remark.deleteRemark(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList remark_list = remark.getRemarkByName(search, n1, recordsperpage);
                int count = 0;
                if (remark_list.size() > 0) 
                {
                    RemarkInfo cinfo = (RemarkInfo) remark_list.get(remark_list.size() - 1);
                    count = cinfo.getRemarkId();
                    remark_list.remove(remark_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("REMARK_LIST", remark_list);
                ...
                if (total > 0) {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + remarkCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) 
                    {
                        ...
                        if (info != null) 
                        {
                            ...
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRemarkId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRemarkId()+"');\">" + remark.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getRemarkId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getRemarkId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + remark.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    remark_list = remark.getRemarkByName(search, n, recordsperpage);
                    if (remark_list.size() > 0) {
                        RemarkInfo cinfo = (RemarkInfo) remark_list.get(remark_list.size() - 1);
                        count = cinfo.getRemarkId();
                        remark_list.remove(remark_list.size() - 1);
                ...
                if (n == 0 && remarkCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && remarkCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
