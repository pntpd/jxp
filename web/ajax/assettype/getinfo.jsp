    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = assettype.deleteAssettype(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList assettype_list = assettype.getAssettypeByName(search, n1, recordsperpage);
                int count = 0;
                if (assettype_list.size() > 0) 
                {
                    AssettypeInfo cinfo = (AssettypeInfo) assettype_list.get(assettype_list.size() - 1);
                    count = cinfo.getAssettypeId();
                    assettype_list.remove(assettype_list.size() - 1);
                }
                ...
                if (n1 == 0 && assettypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && assettypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + assettypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + assettype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    assettype_list = assettype.getAssettypeByName(search, n, recordsperpage);
                    if (assettype_list.size() > 0) {
                        AssettypeInfo cinfo = (AssettypeInfo) assettype_list.get(assettype_list.size() - 1);
                        count = cinfo.getAssettypeId();
                        assettype_list.remove(assettype_list.size() - 1);
                ...
                if (n == 0 && assettypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && assettypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
