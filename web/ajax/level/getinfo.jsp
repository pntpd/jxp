...
<%@page import="com.web.jxp.level.LevelInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="level" class="com.web.jxp.level.Level" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = level.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = level.deleteLevel(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = level.getCount();
                ArrayList level_list = level.getLevelByName(search,n1, recordsperpage);
                int count = 0;
                if (level_list.size() > 0) {
                    LevelInfo cinfo = (LevelInfo) level_list.get(level_list.size() - 1);
                    count = cinfo.getLevelId();
                    level_list.remove(level_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("LEVEL_LIST", level_list);
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = levelCount / recordsperpage;
                    if (levelCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + levelCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (LevelInfo) level_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getLevelId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getLevelId()+"');\">" + level.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getLevelId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getLevelId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + level.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            } else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    level_list = level.getLevelByName(search, n, recordsperpage);
                    if (level_list.size() > 0) {
                        LevelInfo cinfo = (LevelInfo) level_list.get(level_list.size() - 1);
                        count = cinfo.getLevelId();
                        level_list.remove(level_list.size() - 1);
                ...
                if (n == 0 && levelCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && levelCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
