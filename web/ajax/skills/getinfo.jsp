    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = skills.deleteSkills(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList skills_list = skills.getSkillsByName(search, n1, recordsperpage);
                int count = 0;
                if (skills_list.size() > 0) {
                    SkillsInfo cinfo = (SkillsInfo) skills_list.get(skills_list.size() - 1);
                    count = cinfo.getSkillsId();
                    skills_list.remove(skills_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("SKILLS_LIST", skills_list);
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + skillsCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) 
                    {
                        ...
                        if (info != null) 
                        {
                            ...
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getSkillsId() + "');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getSkillsId() + "');\">" + skills.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getSkillsId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getSkillsId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + skills.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    skills_list = skills.getSkillsByName(search, n, recordsperpage);
                    if (skills_list.size() > 0) {
                        SkillsInfo cinfo = (SkillsInfo) skills_list.get(skills_list.size() - 1);
                        count = cinfo.getSkillsId();
                        skills_list.remove(skills_list.size() - 1);
                ...
                if (n == 0 && skillsCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && skillsCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
