...
<%@page import="com.web.jxp.rejectionreason.RejectionreasonInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="rejectionreason" class="com.web.jxp.rejectionreason.Rejectionreason" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = rejectionreason.getCountList("show_size_list");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Type</b> </span> </th>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = rejectionreason.deleteRejectionreason(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = rejectionreason.getCount();
                ArrayList rejectionreason_list = rejectionreason.getRejectionreasonByName(search, n1, recordsperpage);
                int count = 0;
                if (rejectionreason_list.size() > 0) {
                    RejectionreasonInfo cinfo = (RejectionreasonInfo) rejectionreason_list.get(rejectionreason_list.size() - 1);
                    count = cinfo.getRejectionreasonId();
                    rejectionreason_list.remove(rejectionreason_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("REJECTIONREASON_LIST", rejectionreason_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int rejectionreasonCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = rejectionreason_list.size();
                ...
                if (n1 == 0 && rejectionreasonCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && rejectionreasonCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = rejectionreasonCount;
                } else {
                    recordsperpage = rejectionreasonCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = rejectionreasonCount / recordsperpage;
                    if (rejectionreasonCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + rejectionreasonCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (RejectionreasonInfo) rejectionreason_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRejectionreasonId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRejectionreasonId()+"');\">" + (info.getRejectionType()== 1  ? "Rejection" : "Declined" ) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getRejectionreasonId()+"');\">" + rejectionreason.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getRejectionreasonId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getRejectionreasonId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + rejectionreason.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = rejectionreason.getCount();
                ArrayList rejectionreason_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    rejectionreason_list = rejectionreason.getRejectionreasonByName(search, n, recordsperpage);
                    if (rejectionreason_list.size() > 0) {
                        RejectionreasonInfo cinfo = (RejectionreasonInfo) rejectionreason_list.get(rejectionreason_list.size() - 1);
                        count = cinfo.getRejectionreasonId();
                        rejectionreason_list.remove(rejectionreason_list.size() - 1);
                ...
                if (n == 0 && rejectionreasonCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && rejectionreasonCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
