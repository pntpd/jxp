...
<%@page import="com.web.jxp.bankaccounttype.BankAccountTypeInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="bankAccountType" class="com.web.jxp.bankaccounttype.BankAccountType" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = bankAccountType.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = bankAccountType.deleteBankAccountType(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = bankAccountType.getCount();
                ArrayList bankAccountType_list = bankAccountType.getBankAccountTypeByName(search, n1, recordsperpage);
                int count = 0;
                if (bankAccountType_list.size() > 0) {
                    BankAccountTypeInfo cinfo = (BankAccountTypeInfo) bankAccountType_list.get(bankAccountType_list.size() - 1);
                    count = cinfo.getBankAccountTypeId();
                    bankAccountType_list.remove(bankAccountType_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("BANKACCOUNTTYPE_LIST", bankAccountType_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int bankAccountTypeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = bankAccountType_list.size();
                ...
                if (n1 == 0 && bankAccountTypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && bankAccountTypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = bankAccountTypeCount;
                } else {
                    recordsperpage = bankAccountTypeCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = bankAccountTypeCount / recordsperpage;
                    if (bankAccountTypeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + bankAccountTypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (BankAccountTypeInfo) bankAccountType_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankAccountTypeId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getBankAccountTypeId()+"');\">" + bankAccountType.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getBankAccountTypeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getBankAccountTypeId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + bankAccountType.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = bankAccountType.getCount();
                ArrayList bankAccountType_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    bankAccountType_list = bankAccountType.getBankAccountTypeByName(search, n, recordsperpage);
                    if (bankAccountType_list.size() > 0) {
                        BankAccountTypeInfo cinfo = (BankAccountTypeInfo) bankAccountType_list.get(bankAccountType_list.size() - 1);
                        count = cinfo.getBankAccountTypeId();
                        bankAccountType_list.remove(bankAccountType_list.size() - 1);
                ...
                if (n == 0 && bankAccountTypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && bankAccountTypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
