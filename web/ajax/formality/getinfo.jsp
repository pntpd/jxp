    ...
    try
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = formality.deleteFormality(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList formality_list = formality.getFormalityByName(search, n1, recordsperpage,allclient, per, cids, assetids);
                int count = 0;
                if (formality_list.size() > 0) {
                    FormalityInfo cinfo = (FormalityInfo) formality_list.get(formality_list.size() - 1);
                    count = cinfo.getFormalityId();
                    formality_list.remove(formality_list.size() - 1);
                }
                ...
                if (n1 == 0 && formalityCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && formalityCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + formalityCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + formality.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    formality_list = formality.getFormalityByName(search, n, recordsperpage,allclient, per, cids, assetids);
                    if (formality_list.size() > 0) {
                        FormalityInfo cinfo = (FormalityInfo) formality_list.get(formality_list.size() - 1);
                        count = cinfo.getFormalityId();
                        formality_list.remove(formality_list.size() - 1);
                ...
                if (n == 0 && formalityCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && formalityCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
