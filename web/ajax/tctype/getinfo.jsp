...
<%@page import="com.web.jxp.tctype.TctypeInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="tctype" class="com.web.jxp.tctype.Tctype" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = tctype.getCountList("show_size_list");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Asset Name</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Course Type</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = tctype.deleteTctype(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = tctype.getCount();
                ArrayList tctype_list = tctype.getTctypeByName(search, n1, recordsperpage,clientIndex, assetIndex,allclient, per, cids, assetids);
                int count = 0;
                if (tctype_list.size() > 0) {
                    TctypeInfo cinfo = (TctypeInfo) tctype_list.get(tctype_list.size() - 1);
                    count = cinfo.getTctypeId();
                    tctype_list.remove(tctype_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("TCTYPE_LIST", tctype_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int tctypeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = tctype_list.size();
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = tctypeCount / recordsperpage;
                    if (tctypeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + tctypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (TctypeInfo) tctype_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTctypeId()+"');\">" + (info.getClientName()!= null ? info.getClientName(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTctypeId()+"');\">" + (info.getAssetName()!= null ? info.getAssetName(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTctypeId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getTctypeId()+"');\">" + tctype.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getTctypeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getTctypeId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + tctype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = tctype.getCount();
                ArrayList tctype_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    tctype_list = tctype.getTctypeByName(search, n, recordsperpage,clientIndex, assetIndex, allclient, per, cids, assetids);
                    if (tctype_list.size() > 0) {
                        TctypeInfo cinfo = (TctypeInfo) tctype_list.get(tctype_list.size() - 1);
                        count = cinfo.getTctypeId();
                        tctype_list.remove(tctype_list.size() - 1);
                ...
                if (n == 0 && tctypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && tctypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                ...
                if (total > 0) 
                {
                    ...
                    str.append("<table id='tech-companies-1' class='table table-striped'>");
                    str.append(thead);
                    str.append("<tbody id = 'sort_id' id='printArea'>");
