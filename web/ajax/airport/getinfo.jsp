...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Airport</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Shortname</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = airport.deleteAirport(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList airport_list = airport.getAirportByName(search,  n1, recordsperpage);
                int count = 0;
                if (airport_list.size() > 0) {
                    AirportInfo cinfo = (AirportInfo) airport_list.get(airport_list.size() - 1);
                    count = cinfo.getAirportId();
                    airport_list.remove(airport_list.size() - 1);
                }
                ...
                if (n1 == 0 && airportCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && airportCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + airportCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + airport.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    airport_list = airport.getAirportByName(search, n, recordsperpage);
                    if (airport_list.size() > 0) {
                        AirportInfo cinfo = (AirportInfo) airport_list.get(airport_list.size() - 1);
                        count = cinfo.getAirportId();
                        airport_list.remove(airport_list.size() - 1);
                ...
                if (n == 0 && airportCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && airportCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
