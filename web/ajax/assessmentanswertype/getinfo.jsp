...
<%@page import="com.web.jxp.assessmentanswertype.AssessmentAnswerTypeInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="assessmentAnswerType" class="com.web.jxp.assessmentanswertype.AssessmentAnswerType" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = assessmentAnswerType.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = assessmentAnswerType.deleteAssessmentAnswerType(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = assessmentAnswerType.getCount();
                ArrayList assessmentAnswerType_list = assessmentAnswerType.getAssessmentAnswerTypeByName(search, n1, recordsperpage);
                int count = 0;
                if (assessmentAnswerType_list.size() > 0) {
                    AssessmentAnswerTypeInfo cinfo = (AssessmentAnswerTypeInfo) assessmentAnswerType_list.get(assessmentAnswerType_list.size() - 1);
                    count = cinfo.getAssessmentAnswerTypeId();
                    assessmentAnswerType_list.remove(assessmentAnswerType_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("ASSESSMENTANSWERTYPE_LIST", assessmentAnswerType_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int assessmentAnswerTypeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = assessmentAnswerType_list.size();
                ...
                if (n1 == 0 && assessmentAnswerTypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && assessmentAnswerTypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = assessmentAnswerTypeCount;
                } else {
                    recordsperpage = assessmentAnswerTypeCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = assessmentAnswerTypeCount / recordsperpage;
                    if (assessmentAnswerTypeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + assessmentAnswerTypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (AssessmentAnswerTypeInfo) assessmentAnswerType_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentAnswerTypeId()+"');\">" + (assessmentAnswerType.changeNum(info.getAssessmentAnswerTypeId(), 3)) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentAnswerTypeId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentAnswerTypeId()+"');\">" + assessmentAnswerType.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getAssessmentAnswerTypeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getAssessmentAnswerTypeId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + assessmentAnswerType.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = assessmentAnswerType.getCount();
                ArrayList assessmentAnswerType_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    assessmentAnswerType_list = assessmentAnswerType.getAssessmentAnswerTypeByName(search, n, recordsperpage);
                    if (assessmentAnswerType_list.size() > 0) {
                        AssessmentAnswerTypeInfo cinfo = (AssessmentAnswerTypeInfo) assessmentAnswerType_list.get(assessmentAnswerType_list.size() - 1);
                        count = cinfo.getAssessmentAnswerTypeId();
                        assessmentAnswerType_list.remove(assessmentAnswerType_list.size() - 1);
                ...
                if (n == 0 && assessmentAnswerTypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && assessmentAnswerTypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
