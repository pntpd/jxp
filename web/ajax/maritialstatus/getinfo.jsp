...
<%@page import="com.web.jxp.maritialstatus.MaritialStatusInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="maritialstatus" class="com.web.jxp.maritialstatus.MaritialStatus" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = maritialstatus.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = maritialstatus.deleteMaritialStatus(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = maritialstatus.getCount();
                ArrayList maritialstatus_list = maritialstatus.getMaritialStatusByName(search, n1, recordsperpage);
                int count = 0;
                if (maritialstatus_list.size() > 0) {
                    MaritialStatusInfo cinfo = (MaritialStatusInfo) maritialstatus_list.get(maritialstatus_list.size() - 1);
                    count = cinfo.getMaritialStatusId();
                    maritialstatus_list.remove(maritialstatus_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("MARITIALSTATUS_LIST", maritialstatus_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int maritialstatusCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = maritialstatus_list.size();
                ...
                if (n1 == 0 && maritialstatusCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && maritialstatusCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = maritialstatusCount;
                } else {
                    recordsperpage = maritialstatusCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = maritialstatusCount / recordsperpage;
                    if (maritialstatusCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + maritialstatusCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (MaritialStatusInfo) maritialstatus_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+info.getMaritialStatusId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+info.getMaritialStatusId()+"');\">" + maritialstatus.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getMaritialStatusId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getMaritialStatusId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + maritialstatus.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = maritialstatus.getCount();
                ArrayList maritialstatus_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    maritialstatus_list = maritialstatus.getMaritialStatusByName(search, n, recordsperpage);
                    if (maritialstatus_list.size() > 0) {
                        MaritialStatusInfo cinfo = (MaritialStatusInfo) maritialstatus_list.get(maritialstatus_list.size() - 1);
                        count = cinfo.getMaritialStatusId();
                        maritialstatus_list.remove(maritialstatus_list.size() - 1);
                ...
                if (n == 0 && maritialstatusCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && maritialstatusCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
