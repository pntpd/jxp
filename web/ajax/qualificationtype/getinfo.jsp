...
<%@page import="com.web.jxp.qualificationtype.QualificationTypeInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="qualificationtype" class="com.web.jxp.qualificationtype.QualificationType" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = qualificationtype.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = qualificationtype.deleteQualificationType(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = qualificationtype.getCount();
                ArrayList qualificationtype_list = qualificationtype.getQualificationTypeByName(search, n1, recordsperpage);
                int count = 0;
                if (qualificationtype_list.size() > 0) {
                    QualificationTypeInfo cinfo = (QualificationTypeInfo) qualificationtype_list.get(qualificationtype_list.size() - 1);
                    count = cinfo.getQualificationtypeId();
                    qualificationtype_list.remove(qualificationtype_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("QUALIFICATIONTYPE_LIST", qualificationtype_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int qualificationtypeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = qualificationtype_list.size();
                ...
                if (n1 == 0 && qualificationtypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && qualificationtypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = qualificationtypeCount;
                } else {
                    recordsperpage = qualificationtypeCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = qualificationtypeCount / recordsperpage;
                    if (qualificationtypeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + qualificationtypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (QualificationTypeInfo) qualificationtype_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getQualificationtypeId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getQualificationtypeId()+"');\">" + qualificationtype.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getQualificationtypeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getQualificationtypeId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + qualificationtype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = qualificationtype.getCount();
                ArrayList qualificationtype_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    qualificationtype_list = qualificationtype.getQualificationTypeByName(search, n, recordsperpage);
                    if (qualificationtype_list.size() > 0) {
                        QualificationTypeInfo cinfo = (QualificationTypeInfo) qualificationtype_list.get(qualificationtype_list.size() - 1);
                        count = cinfo.getQualificationtypeId();
                        qualificationtype_list.remove(qualificationtype_list.size() - 1);
                ...
                if (n == 0 && qualificationtypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && qualificationtypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
