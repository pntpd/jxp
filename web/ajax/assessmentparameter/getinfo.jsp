...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = assessmentParameter.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = assessmentParameter.deleteAssessmentParameter(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = assessmentParameter.getCount();
                ArrayList assessmentParameter_list = assessmentParameter.getAssessmentParameterByName(search, n1, recordsperpage);
                int count = 0;
                if (assessmentParameter_list.size() > 0) {
                    AssessmentParameterInfo cinfo = (AssessmentParameterInfo) assessmentParameter_list.get(assessmentParameter_list.size() - 1);
                    count = cinfo.getAssessmentParameterId();
                    assessmentParameter_list.remove(assessmentParameter_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("ASSESSMENTPARAMETER_LIST", assessmentParameter_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int assessmentParameterCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = assessmentParameter_list.size();
                ...
                if (n1 == 0 && assessmentParameterCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && assessmentParameterCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = assessmentParameterCount;
                } else {
                    recordsperpage = assessmentParameterCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = assessmentParameterCount / recordsperpage;
                    if (assessmentParameterCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + assessmentParameterCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (AssessmentParameterInfo) assessmentParameter_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentParameterId()+"');\">" + (assessmentParameter.changeNum(info.getAssessmentParameterId(), 3)) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentParameterId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAssessmentParameterId()+"');\">" + assessmentParameter.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getAssessmentParameterId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getAssessmentParameterId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + assessmentParameter.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null)
            {
                ...
                int recordsperpage = assessmentParameter.getCount();
                ArrayList assessmentParameter_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    assessmentParameter_list = assessmentParameter.getAssessmentParameterByName(search, n, recordsperpage);
                    if (assessmentParameter_list.size() > 0) {
                        AssessmentParameterInfo cinfo = (AssessmentParameterInfo) assessmentParameter_list.get(assessmentParameter_list.size() - 1);
                        count = cinfo.getAssessmentParameterId();
                        assessmentParameter_list.remove(assessmentParameter_list.size() - 1);
                ...
                if (n == 0 && assessmentParameterCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && assessmentParameterCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
