...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>State</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = state.deleteState(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList state_list = state.getStateByName(search, n1, recordsperpage);
                int count = 0;
                if (state_list.size() > 0) {
                    StateInfo cinfo = (StateInfo) state_list.get(state_list.size() - 1);
                    count = cinfo.getStateId();
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + stateCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    for (int i = 0; i < total; i++) 
                    {
                        ...
                        if (info != null) 
                        {
                            ...
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getStateId()+"');\">" + (info.getStateName() != null ? info.getStateName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getStateId()+"');\">" + (info.getCountryName() != null ? info.getCountryName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getStateId()+"');\">" + (state.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getStateId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getStateId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + state.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    state_list = state.getStateByName(search, n, recordsperpage);
                    if (state_list.size() > 0) {
                        StateInfo cinfo = (StateInfo) state_list.get(state_list.size() - 1);
                        count = cinfo.getStateId();
                        state_list.remove(state_list.size() - 1);
                ...
                if (n == 0 && stateCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && stateCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
