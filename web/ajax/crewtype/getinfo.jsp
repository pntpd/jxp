...
<%@page import="com.web.jxp.crewtype.CrewtypeInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="crewtype" class="com.web.jxp.crewtype.Crewtype" scope="page"/>
...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = crewtype.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = crewtype.deleteCrewtype(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = crewtype.getCount();
                ArrayList crewtype_list = crewtype.getCrewtypeByName(search, n1, recordsperpage);
                int count = 0;
                if (crewtype_list.size() > 0) {
                    CrewtypeInfo cinfo = (CrewtypeInfo) crewtype_list.get(crewtype_list.size() - 1);
                    count = cinfo.getCrewtypeId();
                    crewtype_list.remove(crewtype_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("CREWTYPE_LIST", crewtype_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int crewtypeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = crewtype_list.size();
                ...
                if (n1 == 0 && crewtypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && crewtypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = crewtypeCount / recordsperpage;
                    if (crewtypeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + crewtypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (CrewtypeInfo) crewtype_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCrewtypeId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCrewtypeId()+"');\">" + crewtype.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getCrewtypeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getCrewtypeId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + crewtype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = crewtype.getCount();
                ArrayList crewtype_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    crewtype_list = crewtype.getCrewtypeByName(search, n, recordsperpage);
                    if (crewtype_list.size() > 0) {
                        CrewtypeInfo cinfo = (CrewtypeInfo) crewtype_list.get(crewtype_list.size() - 1);
                        count = cinfo.getCrewtypeId();
                        crewtype_list.remove(crewtype_list.size() - 1);
                ...
                if (n == 0 && crewtypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && crewtypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
