    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = city.deleteCity(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList city_list = city.getCityByName(search, n1, recordsperpage);
                int count = 0;
                if (city_list.size() > 0) {
                    CityInfo cinfo = (CityInfo) city_list.get(city_list.size() - 1);
                    count = cinfo.getCityId();
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + cityCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + city.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    city_list = city.getCityByName(search, n, recordsperpage);
                    if (city_list.size() > 0) {
                        CityInfo cinfo = (CityInfo) city_list.get(city_list.size() - 1);
                        count = cinfo.getCityId();
                        city_list.remove(city_list.size() - 1);
                ...
                if (n == 0 && cityCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && cityCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
