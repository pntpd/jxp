...
<%@page import="com.web.jxp.companyindustry.CompanyindustryInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="companyindustry" class="com.web.jxp.companyindustry.Companyindustry" scope="page"/>
...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = companyindustry.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = companyindustry.deleteCompanyindustry(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = companyindustry.getCount();
                ArrayList companyindustry_list = companyindustry.getCompanyindustryByName(search, n1, recordsperpage);
                int count = 0;
                if (companyindustry_list.size() > 0) {
                    CompanyindustryInfo cinfo = (CompanyindustryInfo) companyindustry_list.get(companyindustry_list.size() - 1);
                    count = cinfo.getCompanyindustryId();
                    companyindustry_list.remove(companyindustry_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("COMPANYINDUSTRY_LIST", companyindustry_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int companyindustryCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = companyindustry_list.size();
                ...
                if (n1 == 0 && companyindustryCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && companyindustryCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = companyindustryCount;
                } else {
                    recordsperpage = companyindustryCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = companyindustryCount / recordsperpage;
                    if (companyindustryCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + companyindustryCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (CompanyindustryInfo) companyindustry_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCompanyindustryId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getCompanyindustryId()+"');\">" + (companyindustry.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getCompanyindustryId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getCompanyindustryId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + companyindustry.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = companyindustry.getCount();
                ArrayList companyindustry_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    companyindustry_list = companyindustry.getCompanyindustryByName(search, n, recordsperpage);
                    if (companyindustry_list.size() > 0) {
                        CompanyindustryInfo cinfo = (CompanyindustryInfo) companyindustry_list.get(companyindustry_list.size() - 1);
                        count = cinfo.getCompanyindustryId();
                        companyindustry_list.remove(companyindustry_list.size() - 1);
                ...
                if (n == 0 && companyindustryCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && companyindustryCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
