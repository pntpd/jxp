...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = rotation.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>No. of Days</b> </span></th>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = rotation.deleteRotation(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = rotation.getCount();
                ArrayList rotation_list = rotation.getRotationByName(search, n1, recordsperpage);
                int count = 0;
                if (rotation_list.size() > 0) {
                    RotationInfo cinfo = (RotationInfo) rotation_list.get(rotation_list.size() - 1);
                    count = cinfo.getRotationId();
                    rotation_list.remove(rotation_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("ROTATION_LIST", rotation_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int rotationCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = rotation_list.size();
                ...
                if (n1 == 0 && rotationCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && rotationCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = rotationCount / recordsperpage;
                    if (rotationCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + rotationCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (RotationInfo) rotation_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getRotationId() + "');\">" + (info.getNoOfDays()) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getRotationId() + "');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getRotationId() + "');\">" + rotation.getStatusById(info.getStatus()) + "</td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getRotationId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getRotationId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + rotation.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = rotation.getCount();
                ArrayList rotation_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    rotation_list = rotation.getRotationByName(search, n, recordsperpage);
                    if (rotation_list.size() > 0) {
                        RotationInfo cinfo = (RotationInfo) rotation_list.get(rotation_list.size() - 1);
                        count = cinfo.getRotationId();
                        rotation_list.remove(rotation_list.size() - 1);
                ...
                if (n == 0 && rotationCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && rotationCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
