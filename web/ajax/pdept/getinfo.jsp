    ...
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                String statusIndex_s = request.getParameter("statusIndex") != null && !request.getParameter("statusIndex").equals("") ? vobj.replaceint(request.getParameter("statusIndex")) : "0";
                ...
                int result = pdept.deletePdept(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList pdept_list = pdept.getPdeptByName(search, statusIndex, assettypeIdIndex, n1, recordsperpage);
                int count = 0;
                if (pdept_list.size() > 0) {
                    PdeptInfo cinfo = (PdeptInfo) pdept_list.get(pdept_list.size() - 1);
                    count = cinfo.getPdeptId();
                    pdept_list.remove(pdept_list.size() - 1);
                }
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + pdeptCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + pdept.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    pdept_list = pdept.getPdeptByName(search, statusIndex, assettypeIdIndex, n, recordsperpage); 
                    if (pdept_list.size() > 0) {
                        PdeptInfo cinfo = (PdeptInfo) pdept_list.get(pdept_list.size() - 1);
                        count = cinfo.getPdeptId();
                        pdept_list.remove(pdept_list.size() - 1);
                ...
                if (n == 0 && pdeptCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && pdeptCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
