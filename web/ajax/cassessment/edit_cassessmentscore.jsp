        ...
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("cassessmentid") != null) 
            {
                ...
                if (info != null) 
                {
                    ...
                    list = cassessment.getScoreByAssessmentId(cassessmentId);
                    int total = list.size();
                    if (total > 0) 
                    {
                        ...
                        for (int i = 0; i < total; i++) 
                        {
                            ...
                            if (pinfo != null) 
                            {
                                ...
                                sb.append("<td><input type='text' class='form-control' id='marks" + (i + 1) + "' name='marks' value='" + pinfo.getMarks() + "' placeholder='' maxlength='5' onpaste='return false' onkeypress='return allowPositiveNumber(event);' onkeyup='setMarks()' autocomplete='off'/></td>");
                        ...
                        sb.append("<span class='avg_scrore_value");
                        if (info.getPassingFlag() > 0) {
                            if (info.getMarks() >= info.getMinScore()) {
                                sb.append(" passed_mark");
                            }
                        } else {
                            sb.append(" passed_mark");
                        }
                        sb.append("' id='smarks'>" + info.getMarks() + "</span>");
                        ...
                        if (per.equals("Y")) {
                            sb.append("<a href=\"javascript: submitScoreForm();\" class='save_btn'><img src='../assets/images/save.png'> Save</a>");
                        }
                        sb.append("<a href=\"javascript: getViewEditData('" + info.getpAssessmentId() + "', '" + cassessmentId + "','view');\" class='cl_btn score_close_btn'><i class='ion ion-md-close'></i> Close</a>");
