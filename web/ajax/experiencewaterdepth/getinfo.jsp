...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = experiencewaterdepth.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Unit Measurement</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Depth</b> </span></th>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = experiencewaterdepth.deleteExperienceWaterDepth(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = experiencewaterdepth.getCount();
                ArrayList experiencewaterdepth_list = experiencewaterdepth.getExperienceWaterDepthByName(search, n1, recordsperpage);
                int count = 0;
                if (experiencewaterdepth_list.size() > 0) {
                    ExperienceWaterDepthInfo cinfo = (ExperienceWaterDepthInfo) experiencewaterdepth_list.get(experiencewaterdepth_list.size() - 1);
                    count = cinfo.getExperiencewaterdepthId();
                    experiencewaterdepth_list.remove(experiencewaterdepth_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("EXPERIENCEWATERDEPTH_LIST", experiencewaterdepth_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int experiencewaterdepthCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = experiencewaterdepth_list.size();
                ...
                if (n1 == 0 && experiencewaterdepthCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && experiencewaterdepthCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = experiencewaterdepthCount;
                } else {
                    recordsperpage = experiencewaterdepthCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = experiencewaterdepthCount / recordsperpage;
                    if (experiencewaterdepthCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + experiencewaterdepthCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (ExperienceWaterDepthInfo) experiencewaterdepth_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getExperiencewaterdepthId() + "');\">" + (info.getUnitMeasurement() != null ? info.getUnitMeasurement() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getExperiencewaterdepthId() + "');\">" + (info.getDepth()) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('" + info.getExperiencewaterdepthId() + "');\">" + experiencewaterdepth.getStatusById(info.getStatus()) + "</td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getExperiencewaterdepthId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getExperiencewaterdepthId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + experiencewaterdepth.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = experiencewaterdepth.getCount();
                ArrayList experiencewaterdepth_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    experiencewaterdepth_list = experiencewaterdepth.getExperienceWaterDepthByName(search, n, recordsperpage);
                    if (experiencewaterdepth_list.size() > 0) {
                        ExperienceWaterDepthInfo cinfo = (ExperienceWaterDepthInfo) experiencewaterdepth_list.get(experiencewaterdepth_list.size() - 1);
                        count = cinfo.getExperiencewaterdepthId();
                        experiencewaterdepth_list.remove(experiencewaterdepth_list.size() - 1);
                ...
                if (n == 0 && experiencewaterdepthCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && experiencewaterdepthCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
