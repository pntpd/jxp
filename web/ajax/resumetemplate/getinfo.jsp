...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = resumetemplate.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = resumetemplate.deleteResumetemplate(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = resumetemplate.getCount();
                ArrayList resumetemplate_list = resumetemplate.getResumetemplateByName(search, n1, recordsperpage,allclient, per, cids, assetids);
                int count = 0;
                if (resumetemplate_list.size() > 0) {
                    ResumetemplateInfo cinfo = (ResumetemplateInfo) resumetemplate_list.get(resumetemplate_list.size() - 1);
                    count = cinfo.getResumetemplateId();
                    resumetemplate_list.remove(resumetemplate_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("RESUMETEMPLATE_LIST", resumetemplate_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int resumetemplateCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = resumetemplate_list.size();
                ...
                if (n1 == 0 && resumetemplateCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && resumetemplateCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = resumetemplateCount;
                } else {
                    recordsperpage = resumetemplateCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = resumetemplateCount / recordsperpage;
                    if (resumetemplateCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + resumetemplateCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (ResumetemplateInfo) resumetemplate_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getResumetemplateId()+"');\">" + (info.getClientname()!= null ? info.getClientname(): "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getResumetemplateId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getResumetemplateId()+"');\">" + resumetemplate.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getResumetemplateId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getResumetemplateId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + resumetemplate.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = resumetemplate.getCount();
                ArrayList resumetemplate_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    resumetemplate_list = resumetemplate.getResumetemplateByName(search, n, recordsperpage,allclient,per,cids,assetids);
                    if (resumetemplate_list.size() > 0) {
                        ResumetemplateInfo cinfo = (ResumetemplateInfo) resumetemplate_list.get(resumetemplate_list.size() - 1);
                        count = cinfo.getResumetemplateId();
                        resumetemplate_list.remove(resumetemplate_list.size() - 1);
                ...
                if (n == 0 && resumetemplateCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && resumetemplateCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
