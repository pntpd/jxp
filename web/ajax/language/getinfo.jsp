...
<%@page import="com.web.jxp.language.LanguageInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="language" class="com.web.jxp.language.Language" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            int showsizelist = language.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = language.deleteLanguage(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = language.getCount();
                ArrayList language_list = language.getLanguageByName(search, n1, recordsperpage);
                int count = 0;
                if (language_list.size() > 0) {
                    LanguageInfo cinfo = (LanguageInfo) language_list.get(language_list.size() - 1);
                    count = cinfo.getLanguageId();
                    language_list.remove(language_list.size() - 1);
                }
                ...
                int languageCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = language_list.size();
                ...
                if (n1 == 0 && languageCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && languageCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = languageCount / recordsperpage;
                    if (languageCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + languageCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (LanguageInfo) language_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getLanguageId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getLanguageId()+"');\">" + language.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getLanguageId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getLanguageId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + language.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            } else if (request.getParameter("deleteVal") == null)
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    language_list = language.getLanguageByName(search, n, recordsperpage);
                    if (language_list.size() > 0) {
                        LanguageInfo cinfo = (LanguageInfo) language_list.get(language_list.size() - 1);
                        count = cinfo.getLanguageId();
                        language_list.remove(language_list.size() - 1);
                ...
                if (n == 0 && languageCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && languageCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
