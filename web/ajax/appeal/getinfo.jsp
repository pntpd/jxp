...
<%@page import="com.web.jxp.appeal.AppealInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="appeal" class="com.web.jxp.appeal.Appeal" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = appeal.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = appeal.deleteAppeal(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = appeal.getCount();
                ArrayList appeal_list = appeal.getAppealByName(search, n1, recordsperpage);
                int count = 0;
                if (appeal_list.size() > 0) {
                    AppealInfo cinfo = (AppealInfo) appeal_list.get(appeal_list.size() - 1);
                    count = cinfo.getAppealId();
                    appeal_list.remove(appeal_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("APPEAL_LIST", appeal_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int appealCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = appeal_list.size();
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = appealCount / recordsperpage;
                    if (appealCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + appealCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (AppealInfo) appeal_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor'>");
                            str.append("<td class='hand_cursor' onclick=\"javascript: showDetail('"+ info.getAppealId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' onclick=\"javascript: showDetail('"+ info.getAppealId()+"');\">" + appeal.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getAppealId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getAppealId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + appeal.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = appeal.getCount();
                ArrayList appeal_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    appeal_list = appeal.getAppealByName(search, n, recordsperpage);
                    if (appeal_list.size() > 0) {
                        AppealInfo cinfo = (AppealInfo) appeal_list.get(appeal_list.size() - 1);
                        count = cinfo.getAppealId();
                        appeal_list.remove(appeal_list.size() - 1);
                ...
                if (n == 0 && appealCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && appealCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
