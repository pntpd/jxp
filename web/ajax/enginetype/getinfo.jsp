    ...
    try
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = enginetype.deleteEnginetype(id, uid, statusval, ipAddrStr, iplocal);
                ...
                ArrayList enginetype_list = enginetype.getEnginetypeByName(search, n1, recordsperpage);
                int count = 0;
                if (enginetype_list.size() > 0) {
                    EnginetypeInfo cinfo = (EnginetypeInfo) enginetype_list.get(enginetype_list.size() - 1);
                    count = cinfo.getEnginetypeId();
                    enginetype_list.remove(enginetype_list.size() - 1);
                }
                ...
                if (n1 == 0 && enginetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && enginetypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + enginetypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + enginetype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    enginetype_list = enginetype.getEnginetypeByName(search, n, recordsperpage);
                    if (enginetype_list.size() > 0) {
                        EnginetypeInfo cinfo = (EnginetypeInfo) enginetype_list.get(enginetype_list.size() - 1);
                        count = cinfo.getEnginetypeId();
                        enginetype_list.remove(enginetype_list.size() - 1);
                ...
                if (n == 0 && enginetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && enginetypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
