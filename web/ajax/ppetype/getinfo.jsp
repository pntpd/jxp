...
<%@page import="com.web.jxp.ppetype.PpetypeInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="ppetype" class="com.web.jxp.ppetype.Ppetype" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = ppetype.getCountList("show_size_list");
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>PPE Type</b> </span>");            
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Answer Type</b> </span>");            
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Options</b> </span>");            
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = ppetype.deletePpetype(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = ppetype.getCount();
                ArrayList ppetype_list = ppetype.getPpetypeByName(search, n1, recordsperpage);
                int count = 0;
                if (ppetype_list.size() > 0) {
                    PpetypeInfo cinfo = (PpetypeInfo) ppetype_list.get(ppetype_list.size() - 1);
                    count = cinfo.getPpetypeId();
                    ppetype_list.remove(ppetype_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("PPETYPE_LIST", ppetype_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int ppetypeCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = ppetype_list.size();
                ...
                if (n1 == 0 && ppetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && ppetypeCount > ((n1 * recordsperpage) + recordsperpage)) {
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = ppetypeCount / recordsperpage;
                    if (ppetypeCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + ppetypeCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (PpetypeInfo) ppetype_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor'>");
                            str.append("<td class='hand_cursor' onclick=\"javascript: showDetail('"+ info.getPpetypeId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' onclick=\"javascript: showDetail('"+ info.getPpetypeId()+"');\">" + (info.getTypeId() == 1 ? "Text Field" : "Drop Down Selection") + "</td>");
                            str.append("<td class='hand_cursor' onclick=\"javascript: showDetail('"+ info.getPpetypeId()+"');\">" + (info.getTypeId() == 2 ? info.getAddvalue() : "NA") + "</td>");
                            str.append("<td class='hand_cursor' onclick=\"javascript: showDetail('"+ info.getPpetypeId()+"');\">" + ppetype.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getPpetypeId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getPpetypeId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + ppetype.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = ppetype.getCount();
                ArrayList ppetype_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    ppetype_list = ppetype.getPpetypeByName(search, n, recordsperpage);
                    if (ppetype_list.size() > 0) {
                        PpetypeInfo cinfo = (PpetypeInfo) ppetype_list.get(ppetype_list.size() - 1);
                        count = cinfo.getPpetypeId();
                        ppetype_list.remove(ppetype_list.size() - 1);
                ...
                if (n == 0 && ppetypeCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && ppetypeCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
