...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = matrix.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th width='7%'>");
            sb.append("<span><b>ID</b></span>");
            ...
            sb.append("<span><b>Matrix Name</b></span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = matrix.deleteMatrix(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = matrix.getCount();
                ArrayList matrix_list = matrix.getMatrixByName(search, n1, recordsperpage);
                int count = 0;
                if (matrix_list.size() > 0) {
                    MatrixInfo cinfo = (MatrixInfo) matrix_list.get(matrix_list.size() - 1);
                    count = cinfo.getMatrixId();
                    matrix_list.remove(matrix_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("MATRIX_LIST", matrix_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int matrixCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = matrix_list.size();
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = matrixCount / recordsperpage;
                    if (matrixCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + matrixCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (MatrixInfo) matrix_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getMatrixId()+"');\">" + matrix.changeNum(info.getMatrixId(), 3) + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getMatrixId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getMatrixId()+"');\">" + (info.getAssettypeName() != null ? info.getAssettypeName() : "") + "</td>");
                            str.append("<td class='hand_cursor assets_list text-center'><a href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getMatrixId()+"');\">" + info.getPcount() + "</a></td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getMatrixId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                if(info.getEditcount() > 0)
                                    str.append("<a href=\"javascript: modifyForm('" + info.getMatrixId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                                else
                                    str.append("<a href=\"javascript: modifyForm('" + info.getMatrixId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/link.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + matrix.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = matrix.getCount();
                ArrayList matrix_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    matrix_list = matrix.getMatrixByName(search, n, recordsperpage);
                    if (matrix_list.size() > 0) {
                        MatrixInfo cinfo = (MatrixInfo) matrix_list.get(matrix_list.size() - 1);
                        count = cinfo.getMatrixId();
                        matrix_list.remove(matrix_list.size() - 1);
                ...
                if (n == 0 && matrixCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && matrixCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
