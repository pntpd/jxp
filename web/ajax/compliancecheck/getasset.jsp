            ...
            if (request.getParameter("from") != null) 
            {
                if ("asset".equalsIgnoreCase(request.getParameter("from")))
                {
                    if (request.getParameter("clientIdIndex") != null)
                    {
                        ...
                        Collection coll = compliancecheck.getClientAsset(clientId,assetids, allclient, per);
                        ...
                        if (clientId > 0) 
                        {
                            if (coll.size() > 0) 
                            {
                                ...
                                while (iter.hasNext()) 
                                {
                                    CompliancecheckInfo info = (CompliancecheckInfo) iter.next();
                ...
                } else if ("position".equalsIgnoreCase(request.getParameter("from"))) 
                {
                    if (request.getParameter("assetIdIndex") != null) 
                    {
                        ...
                        response.setContentType("text/html");
                        response.setHeader("Cache-Control", "no-cache");
                        Collection coll = compliancecheck.getPostions(clientassetddl);
                        StringBuffer sb = new StringBuffer();
                        if (clientassetddl > 0) 
                        {
                            if (coll.size() > 0)
