...
<%@page import="com.web.jxp.appealrejection.AppealrejectionInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="appealrejection" class="com.web.jxp.appealrejection.Appealrejection" scope="page"/>
...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = appealrejection.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null)
            {
                ...
                int result = appealrejection.deleteAppealrejection(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = appealrejection.getCount();
                ArrayList appealrejection_list = appealrejection.getAppealrejectionByName(search, n1, recordsperpage);
                int count = 0;
                if (appealrejection_list.size() > 0) {
                    AppealrejectionInfo cinfo = (AppealrejectionInfo) appealrejection_list.get(appealrejection_list.size() - 1);
                    count = cinfo.getAppealrejectionId();
                    appealrejection_list.remove(appealrejection_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("APPEALREJECTION_LIST", appealrejection_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int appealrejectionCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = appealrejection_list.size();
                ...
                if (n1 == 0 && appealrejectionCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && appealrejectionCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = appealrejectionCount;
                } else {
                    recordsperpage = appealrejectionCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = appealrejectionCount / recordsperpage;
                    if (appealrejectionCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + appealrejectionCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (AppealrejectionInfo) appealrejection_list.get(i);
                        if (info != null)
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAppealrejectionId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAppealrejectionId()+"');\">" + appealrejection.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getAppealrejectionId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getAppealrejectionId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + appealrejection.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            } else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = appealrejection.getCount();
                ArrayList appealrejection_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    appealrejection_list = appealrejection.getAppealrejectionByName(search, n, recordsperpage);
                    if (appealrejection_list.size() > 0) {
                        AppealrejectionInfo cinfo = (AppealrejectionInfo) appealrejection_list.get(appealrejection_list.size() - 1);
                        count = cinfo.getAppealrejectionId();
                        appealrejection_list.remove(appealrejection_list.size() - 1);
                ...
                if (n == 0 && appealrejectionCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && appealrejectionCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
