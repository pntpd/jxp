        ...
        if (session.getAttribute("LOGININFO") != null)
        {
            ...
            if (request.getParameter("deleteVal") == null) 
            {
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    trainingmatrix_list = trainingmatrix.getTrainingmatrixByName(search, n, recordsperpage,allclient, per, cids, assetids,clientIdIndex, assetIdIndex);
                    if (trainingmatrix_list.size() > 0) {
                        TrainingmatrixInfo cinfo = (TrainingmatrixInfo) trainingmatrix_list.get(trainingmatrix_list.size() - 1);
                        count = cinfo.getClientId();
                        trainingmatrix_list.remove(trainingmatrix_list.size() - 1);
                    }
                } else {
                    trainingmatrix_list = trainingmatrix.getTrainingmatrixByName(search, n, recordsperpage,allclient, per, cids, assetids,clientIdIndex, assetIdIndex);
                    if (trainingmatrix_list.size() > 0) {
                ...
                if (n == 0 && trainingmatrixCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && trainingmatrixCount > ((n * recordsperpage) + recordsperpage)) {
                ...
                if (total > 0)
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + trainingmatrixCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + trainingmatrix.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
