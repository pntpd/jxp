...
try
{
    if(session.getAttribute("LOGININFO") != null)
    {
        ...
        if(request.getParameter("courseId") != null)
        {
            ...
            if(courseId > 0)
            {
                ArrayList list = knowledgecontent.getcourseattachment(courseId);
                int size = list.size();
                if(size > 0) 
                {
                    String view_trainingfiles = knowledgecontent.getMainPath("view_trainingfiles");
                    String firsturl = "";
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        ...
                        if(info != null)
                        {
                            ...
                            if(deleteper.equals("Y"))
                                sb.append("<a href=\"javascript: delpic('"+info.getCourseattachmentId()+"', '"+courseId+"');\" class='down_trash_img'><img src='../assets/images/trash.png'/></a>");
                            sb.append("</div>");
                            sb.append("</div>");
                            sb.append("</li>"); 
                        }
