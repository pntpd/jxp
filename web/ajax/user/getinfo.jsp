...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = user.getCountList("show_size_list");
            
            StringBuilder sb = new StringBuilder();
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th><span><b>Employee Code</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Full Name</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Contact Number</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Email ID</b> </span>");
            ...
            sb.append("</th>");
            sb.append("<th class='text-center'><span><b>Is Admin</b></span></th>");
            ...
            String nextValue = request.getParameter("nextValue") != null && !request.getParameter("nextValue").equals("") ? request.getParameter("nextValue") : "0";
            String page_value = request.getParameter("page") != null && !request.getParameter("page").equals("") && !request.getParameter("page").equals("Page No") ? request.getParameter("page") : "0";
            String dodirects = request.getParameter("doDirect") != null && !request.getParameter("doDirect").equals("") ? request.getParameter("doDirect") : "-1";
            String clientIndex_ids = request.getParameter("clientIndex") != null && !request.getParameter("clientIndex").equals("") ? request.getParameter("clientIndex") : "-1";
            String permissionIndex = request.getParameter("permissionIndex") != null && !request.getParameter("permissionIndex").equals("") ? request.getParameter("permissionIndex") : "";
            int n = Integer.parseInt(nextValue);
            int dodirect = Integer.parseInt(dodirects);
            int clientIndex = Integer.parseInt(clientIndex_ids);
            if (request.getParameter("deleteVal") != null) 
            {
                
                StringBuilder str = new StringBuilder();
                String nextd = request.getParameter("nextDel") != null && !request.getParameter("nextDel").equals("") ? request.getParameter("nextDel") : "-1";
                String delete_ids = request.getParameter("deleteVal") != null && !request.getParameter("deleteVal").equals("") ? request.getParameter("deleteVal") : "0";
                String status_s = request.getParameter("status") != null && !request.getParameter("status").equals("") ? request.getParameter("status") : "1";
                int id = Integer.parseInt(delete_ids);
                int statusval = Integer.parseInt(status_s);
                int result = user.deleteUser(id, statusval);
                int resultId = Integer.parseInt(delete_ids);
                user.createHistoryAccess(null, uid, ipAddrStr, iplocal, "User Deleted", 1, resultId);
                ...
                int recordsperpage = user.getCount();
                ArrayList user_list = user.getUserListByName(search, userId, n1, recordsperpage,clientIndex,permissionIndex);
                int count = 0;
                if (user_list.size() > 0) {
                    UserInfo cinfo = (UserInfo) user_list.get(user_list.size() - 1);
                    count = cinfo.getUserId();
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = userCount / recordsperpage;
                    if (userCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + userCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    str.append("<td>" + user.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int m = n;
                int next_value = 0;
                int pg_val = Integer.parseInt(page_value);
                ...
                if (next1.equals("p") || next1.equals("n")) {
                    user_list = user.getUserListByName(search, userId, n, recordsperpage,clientIndex,permissionIndex);
                    if (user_list.size() > 0) {
                        UserInfo cinfo = (UserInfo) user_list.get(user_list.size() - 1);
                        count = cinfo.getUserId();
                        user_list.remove(user_list.size() - 1);
                ...
                if (n == 0 && userCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && userCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
