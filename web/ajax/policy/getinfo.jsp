...
<%
    try 
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = policy.getCountList("show_size_list");
            ...
            sb.append("</th>");
            sb.append("<th><span><b>Policy</b> </span>");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = policy.deletePolicy(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = policy.getCount();
                ArrayList policy_list = policy.getPolicyByName(search, n1, recordsperpage,clientIndex, assetIndex,allclient,  per,  cids,  assetids );
                int count = 0;
                if (policy_list.size() > 0) {
                    PolicyInfo cinfo = (PolicyInfo) policy_list.get(policy_list.size() - 1);
                    count = cinfo.getPolicyId();
                    policy_list.remove(policy_list.size() - 1);
                }
                ...
                int policyCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = policy_list.size();
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = policyCount / recordsperpage;
                    if (policyCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + policyCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (PolicyInfo) policy_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getPolicyId()+"');\">" + (info.getClientName() != null ? info.getClientName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getPolicyId()+"');\">" + (info.getAssetName() != null ? info.getAssetName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getPolicyId()+"');\">" + (info.getPolicyName() != null ? info.getPolicyName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getPolicyId()+"');\">" + (policy.getStatusById(info.getStatus())) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getPolicyId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getPolicyId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + policy.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
            ...
            else if (request.getParameter("deleteVal") == null) 
            {
                ...
                int recordsperpage = policy.getCount();
                ArrayList policy_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    policy_list = policy.getPolicyByName(search, n, recordsperpage, clientIndex, assetIndex, allclient, per, cids, assetids);
                    if (policy_list.size() > 0) {
                        PolicyInfo cinfo = (PolicyInfo) policy_list.get(policy_list.size() - 1);
                        count = cinfo.getPolicyId();
                        policy_list.remove(policy_list.size() - 1);
                ...
                if (n == 0 && policyCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && policyCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
