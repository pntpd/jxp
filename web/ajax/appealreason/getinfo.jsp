...
<%@page import="com.web.jxp.appealreason.AppealreasonInfo" %>
<%@page import="com.web.jxp.user.UserInfo" %>
<jsp:useBean id="appealreason" class="com.web.jxp.appealreason.Appealreason" scope="page"/>
...
<%
    try
    {
        if (session.getAttribute("LOGININFO") != null) 
        {
            ...
            int showsizelist = appealreason.getCountList("show_size_list");
            ...
            if (request.getParameter("deleteVal") != null) 
            {
                ...
                int result = appealreason.deleteAppealreason(id, uid, statusval, ipAddrStr, iplocal);
                ...
                int recordsperpage = appealreason.getCount();
                ArrayList appealreason_list = appealreason.getAppealreasonByName(search, n1, recordsperpage);
                int count = 0;
                if (appealreason_list.size() > 0) {
                    AppealreasonInfo cinfo = (AppealreasonInfo) appealreason_list.get(appealreason_list.size() - 1);
                    count = cinfo.getAppealreasonId();
                    appealreason_list.remove(appealreason_list.size() - 1);
                }
                request.getSession().setAttribute("COUNT_LIST", count + "");
                session.setAttribute("APPEALREASON_LIST", appealreason_list);
                int pageSize = count / recordsperpage;
                if (count % recordsperpage > 0) {
                    pageSize = pageSize + 1;
                }

                int appealreasonCount = count;
                String prev = "";
                String prevclose = "";
                String next = "";
                String nextclose = "";
                int last = 0;
                int total = appealreason_list.size();
                ...
                if (n1 == 0 && appealreasonCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n1 > 0 && appealreasonCount > ((n1 * recordsperpage) + recordsperpage)) {
                    ...
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
                    last = appealreasonCount;
                } else {
                    recordsperpage = appealreasonCount;
                }
                ...
                int noOfPages = 1;
                if (recordsperpage > 0) {
                    noOfPages = appealreasonCount / recordsperpage;
                    if (appealreasonCount % recordsperpage > 0) {
                        noOfPages += 1;
                    }
                }
                if (total > 0) 
                {
                    ...
                    str.append("(" + value + " - " + value1 + " record(s) of " + appealreasonCount + ")&nbsp;");
                    str.append("</div>");
                    ...
                    {
                        info = (AppealreasonInfo) appealreason_list.get(i);
                        if (info != null) 
                        {
                            status = info.getStatus();
                            str.append("<tr class='hand_cursor' href='javascript: void(0);'>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAppealreasonId()+"');\">" + (info.getName() != null ? info.getName() : "") + "</td>");
                            str.append("<td class='hand_cursor' href='javascript: void(0);' onclick=\"javascript: showDetail('"+ info.getAppealreasonId()+"');\">" + appealreason.getStatusById(info.getStatus()) + "</td>");
                            str.append("<td>");
                            ...
                            str.append(" onclick=\"javascript: deleteForm('" + info.getAppealreasonId() + "', '" + status + "', '" + (i) + "');\" />");
                            str.append("</div>");
                            str.append("</span>");
                            if (editper.equals("Y") && info.getStatus() == 1) {
                                str.append("<a href=\"javascript: modifyForm('" + info.getAppealreasonId() + "');\" class='edit_mode float-end mr_15'><img src='../assets/images/pencil.png'/></a>");
                            }
                            str.append("</td>");
                            str.append("</tr>");
                        }
                    ...
                    str.append("<td>" + appealreason.getMainPath("record_not_found") + "</td>");
                    str.append("</tr>");
                    str.append("</tbody>");
                    str.append("</table>");
                    str.append("</div>");
                ...
                int recordsperpage = appealreason.getCount();
                ArrayList appealreason_list = new ArrayList();
                int count = 0;
                if (next1.equals("p") || next1.equals("n")) {
                    appealreason_list = appealreason.getAppealreasonByName(search, n, recordsperpage);
                    if (appealreason_list.size() > 0) {
                        AppealreasonInfo cinfo = (AppealreasonInfo) appealreason_list.get(appealreason_list.size() - 1);
                        count = cinfo.getAppealreasonId();
                        appealreason_list.remove(appealreason_list.size() - 1);
                ...
                if (n == 0 && appealreasonCount > recordsperpage) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                } else if (n > 0 && appealreasonCount > ((n * recordsperpage) + recordsperpage)) {
                    next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='Next'><i class='fa fa-angle-right'>");
                    nextclose = ("</i></a></li>");
                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                    prevclose = ("</i></a></li>");
