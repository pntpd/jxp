...
<html lang="en">
    ...
    <%
            ...
            int count = 0;
            int recordsperpage = billingcycle.getCount();
            ...
            int showsizelist = billingcycle.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/billingcycle/BillingcycleAction.do" onsubmit="return false;">
...
<%
                                                                                            ...
                                                                                            ((nextCount*recordsperpage) + recordsperpage))
                                                                                        {
                                                                                            prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                                                            prevclose = ("</i></a></li>");
                                                                                            last = billingcycleCount;
                                                                                        }
                                                                                        else
                                                                                        {
                                                                                            recordsperpage = billingcycleCount;
                                                                                        }
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                    ...
                                                                                    if (recordsperpage > 0)
                                                                                    {
                                                                                        noOfPages = billingcycleCount / recordsperpage;
                                                                                        if (billingcycleCount % recordsperpage > 0)
                                                                                            noOfPages += 1;
                                                                                    }
                                                                                    if(total > 0)
                                                                                    {
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=billingcycleCount%>)                                                
                                                </div>
...
%>
                                                                <tr>
                                                                    ...
                                                                    <td><%= info.getClientAssetName() != null ? info.getClientAssetName() : ""%></td>
                                                                    <td  class="assets_list text-center" data-org-colspan="1" data-columns="tech-companies-1-col-2"><a href="javascript:;"><%= info.getPcount() %></a></td>
                                                                    <td class="text-center"><%= info.getTypevalue() != null ? info.getTypevalue() : ""%></td>
                                                                    <td class="action_column">
                                                                        <% if (editper.equals("Y") || addper.equals("Y")) {%>
                                                                        <a href="javascript: showDetail('<%= info.getClientId()%>','<%= info.getAssetId()%>');" class=" mr_15"><%if(info.getTypevalue() != null && !info.getTypevalue().equals("")){%><img src="../assets/images/pencil.png" /><% } else  {%><img src="../assets/images/link.png" /><% } %></a>
                                                                        <%}%>    
                                                                        <span class="switch_bth">
                                                                                <div class="form-check form-switch">
                                                                                    <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <%= status == 1 ? "checked" : ""%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteFormDetail('<%= info.getClientId()%>','<%= info.getAssetId()%>', '<%=status%>', '<%=i%>');"/>
                                                                                </div>
                                                                            </span>
                                                                    </td>
                                                                </tr>
