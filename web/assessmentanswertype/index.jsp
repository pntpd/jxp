...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("ASSESSMENTANSWERTYPE_LIST") != null)
                assessmentAnswerType_list = (ArrayList) session.getAttribute("ASSESSMENTANSWERTYPE_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= assessmentAnswerType.getMainPath("title") != null ? assessmentAnswerType.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/assessmentanswertype.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/assessmentanswertype/AssessmentAnswerTypeAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="assessmentAnswerTypeId"/>
            <!-- Begin page -->
                        ...
                        <div class="row head_title_area">
                            <div class="col-md-12 col-xl-12">
                                <div class="float-start back_arrow">
                                    ...
                                    <span>Assessment Answer Types</span>
                                </div>
                                ...
                                <div class="col-md-12 col-xl-12">
                                    <div class="body-background mt_0">
                                        <div class="row" id='ajax_cat'>
                                            ...
                                            <%
                                                                                        if (nextCount == 0 && assessmentAnswerTypeCount > recordsperpage)
                                                                                        {
                                                                                            next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                                                                                            nextclose = ("</i></a></li>");
                                                                                        }
                                                                                        else if (nextCount > 0 && assessmentAnswerTypeCount >
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=assessmentAnswerTypeCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAssessmentAnswerTypeId()%>');"><%= assessmentAnswerType.changeNum( info.getAssessmentAnswerTypeId(),3) %></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAssessmentAnswerTypeId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAssessmentAnswerTypeId()%>');"> <%= assessmentAnswerType.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getAssessmentAnswerTypeId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getAssessmentAnswerTypeId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                                                                            }
