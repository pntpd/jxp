...
<html lang="en">
    ...
    <%
            ...
            int count = 0;
            int recordsperpage = city.getCount();
            ...
            int showsizelist = city.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <head>
        <meta charset="utf-8">
        <title><%= city.getMainPath("title") != null ? city.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/city.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/city/CityAction.do" onsubmit="return false;">
                                ...
                                <div class="body-background mt_0">
                                    <div class="row" id='ajax_cat'>
                                        ...
                                        <%
                                                                                ...
                                                                                if (recordsperpage > 0)
                                                                                {
                                                                                    noOfPages = cityCount / recordsperpage;
                                                                                    if (cityCount % recordsperpage > 0)
                                                                                        noOfPages += 1;
                                                                                }
                                                                                if(total > 0)
                                                                                {
                                        ...
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=cityCount%>)                                                
                                            </div>
                                                    ...
                                                    <table id="tech-companies-1" class="table table-striped">
                                                        ...
                                                        <tbody id="sort_id">
                                                            ...
                                                            <tr class='hand_cursor' href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getCityId()%>');"><%= info.getCityName() != null ? info.getCityName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getCityId()%>');"><%= info.getCountryName() != null ? info.getCountryName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getCityId()%>');"><%= city.getStatusById(info.getStatus())%></td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getCityId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getCityId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
                                                            <%
                                                                                                                        }
