...
<%@page import="com.web.jxp.skills.SkillsInfo"%>
<%@taglib uri="/WEB-INF/struts-html-el.tld" prefix="html"%>
<%@taglib uri="/WEB-INF/fmt.tld" prefix="fmt"%>
<%@taglib uri="/WEB-INF/c.tld" prefix="c"%>
<jsp:useBean id="skills" class="com.web.jxp.skills.Skills" scope="page"/>
<!doctype html>
<html lang="en">
<%
    try
...
<%
        ...
        ArrayList skills_list = new ArrayList();
        int count = 0;
        int recordsperpage = skills.getCount();
        if(session.getAttribute("COUNT_LIST") != null)
            count = Integer.parseInt((String) session.getAttribute("COUNT_LIST"));
        if(session.getAttribute("SKILLS_LIST") != null)
            skills_list = (ArrayList) session.getAttribute("SKILLS_LIST");
        int pageSize = count / recordsperpage;
        if(count % recordsperpage > 0)
            pageSize = pageSize + 1;

        int total = skills_list.size();
        int showsizelist = skills.getCountList("show_size_list");
        int CurrPageNo = 1;
...
<head>
    <meta charset="utf-8">
    <title><%= skills.getMainPath("title") != null ? skills.getMainPath("title") : "" %></title>
    ...
    <script type="text/javascript" src="../jsnew/skills.js"></script>
    <script type="text/javascript" src="../jsnew/validation.js"></script>
</head>
<body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/skills/SkillsAction.do" onsubmit="return false;">
    <html:hidden property="doAdd"/>
    <html:hidden property="doModify"/>
    <html:hidden property="doView"/>    
    <html:hidden property="skillsId"/>
        <!-- Begin page -->
...
<%
                                        ...
                                        if (recordsperpage > 0)
                                        {
                                            noOfPages = skillsCount / recordsperpage;
                                            if (skillsCount % recordsperpage > 0)
                                                noOfPages += 1;
                                        }
                                        if(total > 0)
                                        {
%>
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=skillsCount%>)                                                
                                            </div>
...
<%
                                                        ...
                                                        {
                                                            info = (SkillsInfo) skills_list.get(i);
                                                            if (info != null) 
                                                            {
                                                                status = info.getStatus();
%>
                                                            <tr class="hand_cursor" href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getSkillsId()%>');"><%= info.getName() != null ? info.getName() : "" %></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getSkillsId()%>');"><%= skills.getStatusById(info.getStatus()) %></td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getSkillsId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getSkillsId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
<%
                                                            }
