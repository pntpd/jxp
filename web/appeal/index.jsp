...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("APPEAL_LIST") != null)
                appeal_list = (ArrayList) session.getAttribute("APPEAL_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= appeal.getMainPath("title") != null ? appeal.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/appeal.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/appeal/AppealAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="appealId"/>
            <!-- Begin page -->
...
%>
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=appealCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor' >
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAppealId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getAppealId()%>');"> <%= appeal.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getAppealId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getAppealId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                            }
