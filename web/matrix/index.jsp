...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("MATRIX_LIST") != null)
                matrix_list = (ArrayList) session.getAttribute("MATRIX_LIST");
    ...
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/matrix/MatrixAction.do" onsubmit="return false;">
...
<%
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=matrixCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getMatrixId()%>');"><%= matrix.changeNum(info.getMatrixId(), 3)%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getMatrixId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getMatrixId()%>');"><%= info.getAssettypeName() != null ? info.getAssettypeName() : ""%></td>
                                                                    <td class="hand_cursor assets_list text-center"><a href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getMatrixId()%>');"><%= info.getPcount() %></a></td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getMatrixId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: showDetail('<%= info.getMatrixId()%>');" class="edit_mode float-end mr_15"><% if(info.getEditcount() > 0) {%><img src="../assets/images/pencil.png"/><% } else {%><img src="../assets/images/link.png" /><% } %></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                                                                            }
