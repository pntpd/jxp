...
<%
        ...
        ArrayList experiencedept_list = new ArrayList();
        int count = 0;
        int recordsperpage = experiencedept.getCount();
        if(session.getAttribute("COUNT_LIST") != null)
            count = Integer.parseInt((String) session.getAttribute("COUNT_LIST"));
        if(session.getAttribute("EXPERIENCEDEPT_LIST") != null)
            experiencedept_list = (ArrayList) session.getAttribute("EXPERIENCEDEPT_LIST");
        int pageSize = count / recordsperpage;
        if(count % recordsperpage > 0)
            pageSize = pageSize + 1;

        int total = experiencedept_list.size();
        int showsizelist = experiencedept.getCountList("show_size_list");
        int CurrPageNo = 1;
...
<head>
    <meta charset="utf-8">
    <title><%= experiencedept.getMainPath("title") != null ? experiencedept.getMainPath("title") : "" %></title>
    ...
    <script type="text/javascript" src="../jsnew/experiencedept.js"></script>
    <script type="text/javascript" src="../jsnew/validation.js"></script>
</head>
<body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/experiencedept/ExperienceDeptAction.do" onsubmit="return false;">
    <html:hidden property="doAdd"/>
    <html:hidden property="doModify"/>
    <html:hidden property="doView"/>    
    <html:hidden property="experiencedeptId"/>
        <!-- Begin page -->
                    ...
                    <div class="row head_title_area">
                        <div class="col-md-12 col-xl-12">
                            <div class="float-start back_arrow">
                                ...
                                <span>Departments/Functions</span>
                            </div>
...
<%
                                                ...
                                                ((nextCount*recordsperpage) + recordsperpage))
                                            {
                                                prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                prevclose = ("</i></a></li>");
                                                last = experiencedeptCount;
                                            }
                                            else
                                            {
                                                recordsperpage = experiencedeptCount;
                                            }
                                        }
                                        else
                                        {
                                        ...
                                        if (recordsperpage > 0)
                                        {
                                            noOfPages = experiencedeptCount / recordsperpage;
                                            if (experiencedeptCount % recordsperpage > 0)
                                                noOfPages += 1;
                                        }
                                        if(total > 0)
                                        {
%>
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=experiencedeptCount%>)                                                
                                            </div>
...
<%
                                                        ...
                                                        {
                                                            info = (ExperienceDeptInfo) experiencedept_list.get(i);
                                                            if (info != null) 
                                                            {
                                                                status = info.getStatus();
%>
                                                            <tr class='hand_cursor' href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getExperiencedeptId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getExperiencedeptId()%>');"> <%= experiencedept.getStatusById(info.getStatus()) %>  </td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getExperiencedeptId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getExperiencedeptId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
<%
                                                            }
