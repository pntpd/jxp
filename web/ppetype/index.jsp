...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("PPETYPE_LIST") != null)
                ppetype_list = (ArrayList) session.getAttribute("PPETYPE_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= ppetype.getMainPath("title") != null ? ppetype.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/ppetype.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/ppetype/PpetypeAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="ppetypeId"/>
            <!-- Begin page -->
...
%>
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=ppetypeCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor' >
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getPpetypeId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getPpetypeId()%>');"><%= info.getTypeId() == 1 ? "Text Field" : "Drop Down Selection"%></td>
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getPpetypeId()%>');"><%= info.getTypeId() == 2 ? info.getAddvalue() : "NA"%></td>
                                                                    <td class="hand_cursor" onclick="javascript: showDetail('<%= info.getPpetypeId()%>');"> <%= ppetype.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getPpetypeId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getPpetypeId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                            }
