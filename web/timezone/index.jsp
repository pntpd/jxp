...
<%@page import="com.web.jxp.timezone.TimezoneInfo"%>
<%@taglib uri="/WEB-INF/struts-html-el.tld" prefix="html"%>
<%@taglib uri="/WEB-INF/fmt.tld" prefix="fmt"%>
<%@taglib uri="/WEB-INF/c.tld" prefix="c"%>
<jsp:useBean id="timezone" class="com.web.jxp.timezone.Timezone" scope="page"/>
<!doctype html>
<html lang="en">
    <%
        try
    ...
    <%
            ...
            ArrayList timezone_list = new ArrayList();
            int count = 0;
            int recordsperpage = timezone.getCount();
            if(session.getAttribute("COUNT_LIST") != null)
                count = Integer.parseInt((String) session.getAttribute("COUNT_LIST"));
            if(session.getAttribute("TIMEZONE_LIST") != null)
                timezone_list = (ArrayList) session.getAttribute("TIMEZONE_LIST");
            int pageSize = count / recordsperpage;
            if(count % recordsperpage > 0)
                pageSize = pageSize + 1;

            int total = timezone_list.size();
            int showsizelist = timezone.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <head>
        <meta charset="utf-8">
        <title><%= timezone.getMainPath("title") != null ? timezone.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/timezone.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/timezone/TimezoneAction.do" onsubmit="return false;">
        <html:hidden property="doAdd"/>
        <html:hidden property="doModify"/>
        <html:hidden property="doView"/>    
        <html:hidden property="timezoneId"/>
        <!-- Begin page -->
                                            ...
                                            <div class="col-lg-12">
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-9 field_ic">
                                                        <html:text property ="search" styleClass="form-control" styleId="example-text-input" maxlength="200" onkeypress="javascript: handleKeySearch(event);" onfocus="if (this.hasAttribute('readonly')) {
                            ...
                            <div class="col-md-12 col-xl-12">
                                <div class="body-background mt_0">
                                    <div class="row" id='ajax_cat'>
                                        ...
                                        if (recordsperpage > 0)
                                        {
                                            noOfPages = timezoneCount / recordsperpage;
                                            if (timezoneCount % recordsperpage > 0)
                                                noOfPages += 1;
                                        }
                                        if(total > 0)
                                        {
                                        ...
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=timezoneCount%>)                                                
                                            </div>
                                                ...
                                                <div class="table-responsive mb-0" data-bs-pattern="priority-columns">        
                                                    <table id="tech-companies-1" class="table table-striped">
                                                        ...
                                                        <tbody id="sort_id">
                                                            ...
                                                            {
                                                                info = (TimezoneInfo) timezone_list.get(i);
                                                                if (info != null) 
                                                                {
                                                                    status = info.getStatus();
                                                            %>
                                                            <tr class='hand_cursor' href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getTimezoneId()%>');"><%= info.getTimezoneName() != null ? info.getTimezoneName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getTimezoneId()%>');"><%= info.getCountryName() != null ? info.getCountryName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getTimezoneId()%>');"><%= info.getTimezoneCode() != null ? info.getTimezoneCode() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getTimezoneId()%>');"><%= timezone.getStatusById(info.getStatus())%></td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getTimezoneId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getTimezoneId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
                                                            <%
                                                                }
