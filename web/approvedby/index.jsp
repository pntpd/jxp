...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("APPROVEDBY_LIST") != null)
                approvedby_list = (ArrayList) session.getAttribute("APPROVEDBY_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= approvedby.getMainPath("title") != null ? approvedby.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/approvedby.js"></script>
         <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/approvedby/ApprovedbyAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="approvedbyId"/>
            <!-- Begin page -->
                                        ...
                                        <div class="row" id='ajax_cat'>
                                            ...
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=approvedbyCount%>)                                                
                                                </div>
                                                        ...
                                                        <table id="tech-companies-1" class="table table-striped">
                                                            ...
                                                            <tbody id="sort_id">
                                                                ...
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getApprovedbyId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getApprovedbyId()%>');"> <%= approvedby.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getApprovedbyId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getApprovedbyId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                                                                            }
