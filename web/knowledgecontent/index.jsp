...
<html lang="en">
    ...
    <%
        ...
        int count = 0;
        int recordsperpage = knowledgecontent.getCount();
        ...
        int showsizelist = knowledgecontent.getCountList("show_size_list");
        int CurrPageNo = 1;
        ...
        if (request.getAttribute("CATEGORYSAVEMODEL") != null) {
            thankyou = (String) request.getAttribute("CATEGORYSAVEMODEL");
    ...
    <html:form action="/knowledgecontent/KnowledgecontentAction.do" onsubmit="return false;">
                                    ...
                                    <div class="row" id='ajax_cat'>
                                        ...
                                        <%
                                                if (nextCount == 0 && createtrainingCount > recordsperpage) {
                                                    ...
                                                    prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                    prevclose = ("</i></a></li>");
                                                    last = createtrainingCount;
                                                } else {
                                                    recordsperpage = createtrainingCount;
                                                }
                                            } else {
                                                recordsperpage = createtrainingCount;
                                            }
                                            ...
                                            int noOfPages = 1;
                                            if (recordsperpage > 0) {
                                                noOfPages = createtrainingCount / recordsperpage;
                                                if (createtrainingCount % recordsperpage > 0) {
                                                    noOfPages += 1;
                                                }
                                            }
                                            if (total > 0) {
                                        ...
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages%>, &nbsp;(<%= nextCount * recordsperpage + 1%> - <%= last > 0 ? last : nextCount * recordsperpage + recordsperpage%> record(s) of <%=createtrainingCount%>)                                                
                                            </div>
                                                    ...
                                                    <table id="tech-companies-1" class="table table-striped">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th  class="text-center"><span><b>Module</b></span>
                                                                ...
                                                                <th  class="text-center"><span><b>Topics</b></span>
                                                        ...
                                                        <tbody id="sort_id">
                                                            ...
                                                            <tr class="hand_cursor" href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getCategoryId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                <td class="assets_list text-center"><a href="javascript: viewSubcategory('<%= info.getCategoryId()%>');"><%=  knowledgecontent.changeNum(info.getSubcategorycount(), 2)%></a></td>
                                                                <td class="assets_list text-center"><a href="javascript: viewCoursecat('<%= info.getCategoryId()%>','0');"><%=  knowledgecontent.changeNum(info.getCoursecount(), 2)%></a></td>
                                                                <td class="action_column">
                                                                    <% if (addper.equals("Y") && info.getStatus() == 1) {%><a class="mr_15" href=" javascript: addsubcategoryForm('<%=info.getCategoryId()%>');"><img src="../assets/images/plus.png"></a><% }%>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if (status == 1) {%>checked<% }%> <% if (!editper.equals("Y")) {%>disabled="true"<% }%> onclick="javascript: deleteForm('<%= info.getCategoryId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getCategoryId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
                                                            <%
                                                                    }
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <h3>Content Management Category Created</h3>
                                <p>Go ahead and create Module under the same!</p>
                                <a href="javascript:;" onclick="javascript: addsubcategoryForm('-1');" class="msg_button" style="text-decoration: underline;">Create Module</a>
                            </div>
                        </div>
                    </div>
                </div>
