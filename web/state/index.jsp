...
<%@page import="com.web.jxp.state.StateInfo"%>
<%@taglib uri="/WEB-INF/struts-html-el.tld" prefix="html"%>
<%@taglib uri="/WEB-INF/fmt.tld" prefix="fmt"%>
<%@taglib uri="/WEB-INF/c.tld" prefix="c"%>
<jsp:useBean id="state" class="com.web.jxp.state.State" scope="page"/>
<!doctype html>
<html lang="en">
    <%
        try
    ...
    <%
            ...
            int count = 0;
            int recordsperpage = state.getCount();
            ...
            if(session.getAttribute("STATE_LIST") != null)
                state_list = (ArrayList) session.getAttribute("STATE_LIST");
            int pageSize = count / recordsperpage;
            if(count % recordsperpage > 0)
                pageSize = pageSize + 1;

            int total = state_list.size();
            int showsizelist = state.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <head>
        <meta charset="utf-8">
        <title><%= state.getMainPath("title") != null ? state.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/state.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/state/StateAction.do" onsubmit="return false;">
                                ...
                                <div class="body-background mt_0">
                                    <div class="row" id='ajax_cat'>
                                        ...
                                        <%
                                                                                ...
                                                                                if (recordsperpage > 0)
                                                                                {
                                                                                    noOfPages = stateCount / recordsperpage;
                                                                                    if (stateCount % recordsperpage > 0)
                                                                                        noOfPages += 1;
                                                                                }
                                                                                if(total > 0)
                                                                                {
                                        ...
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=stateCount%>)                                                
                                            </div>
                                                    ...
                                                    <table id="tech-companies-1" class="table table-striped">
                                                        ...
                                                        <tbody id="sort_id">
                                                            <%
                                                                                                                    ...
                                                                                                                    {
                                                                                                                        info = (StateInfo) state_list.get(i);
                                                                                                                        if (info != null) 
                                                                                                                        {
                                                                                                                            status = info.getStatus();
                                                            %>
                                                            <tr class='hand_cursor' href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getStateId()%>');"><%= info.getStateName() != null ? info.getStateName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getStateId()%>');"><%= info.getCountryName() != null ? info.getCountryName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getStateId()%>');"><%= state.getStatusById(info.getStatus())%></td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getStateId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getStateId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
                                                            <%
                                                                                                                        }
