...
<%
        ...
        int count = 0;
        int recordsperpage = documentType.getCount();
        ...
        int showsizelist = documentType.getCountList("show_size_list");
        int CurrPageNo = 1;
...
<body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/documenttype/DocumentTypeAction.do" onsubmit="return false;">
                    ...
                    <div class="row head_title_area">
                        <div class="col-md-12 col-xl-12">
                            <div class="float-start back_arrow">
                                ...
                                <span>Required Document List</span>
                            </div>
                            <div class="float-end">
...
<%
                                                ...
                                                ((nextCount*recordsperpage) + recordsperpage))
                                            {
                                                prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                prevclose = ("</i></a></li>");
                                                last = documentTypeCount;
                                            }
                                            else
                                            {
                                                recordsperpage = documentTypeCount;
                                            }
                                        }
                                        else
                                        {
                                        ...
                                        if (recordsperpage > 0)
                                        {
                                            noOfPages = documentTypeCount / recordsperpage;
                                            if (documentTypeCount % recordsperpage > 0)
                                                noOfPages += 1;
                                        }
                                        if(total > 0)
                                        {
%>
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=documentTypeCount%>)                                                
                                            </div>
                                                    ...
                                                    <table id="tech-companies-1" class="table table-striped">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th width="50%">
                                                                    <span><b>Module-Sub-Module</b></span>
                                                                </th>
                                                                <th width="20%">
...
%>
                                                            <tr class='hand_cursor' href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getDocumentTypeId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                ...
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getDocumentTypeId()%>');"><%= documentType.getDocFormatId(info.getDocFormat()) != null ? documentType.getDocFormatId(info.getDocFormat()) : ""%></td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getDocumentTypeId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getDocumentTypeId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
<%
                                                            }
