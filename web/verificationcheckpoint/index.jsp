...
<html lang="en">
    ...
    <%
            ...
            int count = 0;
            int recordsperpage = verificationcheckpoint.getCount();
            ...
            int showsizelist = verificationcheckpoint.getCountList("show_size_list");
            int CurrPageNo = 1;
    ...
    <html:form action="/verificationcheckpoint/VerificationcheckpointAction.do" onsubmit="return false;">
                            ...
                            <div class="col-md-12 col-xl-12">
                                <div class="body-background mt_0">
                                    <div class="row" id='ajax_cat'>
                                        ...
                                        <%
                                                                                int verificationcheckpointCount = count;
                                                                                int nextCount = 0;
                                                                                String prev = "";
                                                                                String prevclose = "";
                                                                                String next = "";
                                        ...
                                        <%
                                                                                    if (nextCount == 0 && verificationcheckpointCount > recordsperpage)
                                                                                    {
                                                                                        next = ("<li class='next'><a href=\"javascript: searchFormAjax('n','-1')\" title='First'><i class='fa fa-angle-right'>");
                                                                                        nextclose = ("</i></a></li>");
                                                                                    }
                                                                                    else if (nextCount > 0 && verificationcheckpointCount >
                                                                                        ...
                                                                                        ((nextCount*recordsperpage) + recordsperpage))
                                                                                    {
                                                                                        prev = ("<li class='prev'><a href=\"javascript: searchFormAjax('p','-1')\" title='Prev'><i class='fa fa-angle-left'>");
                                                                                        prevclose = ("</i></a></li>");
                                                                                        last = verificationcheckpointCount;
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                        recordsperpage = verificationcheckpointCount;
                                                                                    }
                                                                                }
                                                                                else
                                                                                {
                                                                                ...
                                                                                if (recordsperpage > 0)
                                                                                {
                                                                                    noOfPages = verificationcheckpointCount / recordsperpage;
                                                                                    if (verificationcheckpointCount % recordsperpage > 0)
                                                                                        noOfPages += 1;
                                                                                }
                                                                                if(total > 0)
                                                                                {
                                        ...
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=verificationcheckpointCount%>)                                                
                                            </div>
                                                    ...
                                                    <table id="tech-companies-1" class="table table-striped">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th width="50%"><span><b>Display Note</b></span>
                                                                ...
                                                                <th width="14%" class="text-center"><span><b>Minimum Requirement</b></span></th>
                                                        ...
                                                        <tbody id="sort_id">
                                                            ...
                                                            %>
                                                            <tr>
                                                                <td class="hand_cursor" href="javascript: void(0);" onclick="javascript: showDetail('<%= info.getVerificationcheckpointId()%>');" ><%= info.getVerificationcheckpointName() != null ? info.getVerificationcheckpointName() : ""%></td>
                                                                <td class="hand_cursor" href="javascript: void(0);" onclick="javascript: showDetail('<%= info.getVerificationcheckpointId()%>');"><%= info.getTabname() != null ? info.getTabname() : ""%></td>
                                                                <td class="hand_cursor" href="javascript: void(0);" onclick="javascript: showDetail('<%= info.getVerificationcheckpointId()%>');"><%= info.getDisplaynote() != null ? info.getDisplaynote() : ""%></td>
                                                                <% if( info.getMinverification() == 1) {%><td class="primary_active text-center"><i class="fa fa-check" aria-hidden="true"></i></td><% } else {%><td></td><% } %>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getVerificationcheckpointId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getVerificationcheckpointId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
                                                            <%
                                                                                                                        }
                                                                                                                                            ...
                                                                                                                                            {
                                                            %>
                                                            <tr><td colspan='6'>No verification checkpoints available.</td></tr>
                                                            <%
                                                                                                                                    }
                                                            %>
                                                        </tbody>
