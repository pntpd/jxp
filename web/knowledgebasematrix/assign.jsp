...
<html lang="en">
    ...
    <%
        ...
        KnowledgebasematrixInfo pinfo = null;
        String positionName = "";
        if (session.getAttribute("WELLPINFO") != null) {
            pinfo = (KnowledgebasematrixInfo) session.getAttribute("WELLPINFO");
            if (pinfo != null) {
        ...
        if (request.getAttribute("WELLCATNAME") != null) {
            KnowledgebasematrixInfo minfo = (KnowledgebasematrixInfo) request.getAttribute("WELLCATNAME");
            if (minfo != null) {
            ...
            <div class="main-content">
                <div class="page-content">
                    <div class="row head_title_area head_fixed">
                        <div class="col-xl-12">
                            <div class="float-start"><a href="javascript: gobackassign();" class="back_arrow"><img  src="../assets/images/back-arrow.png"/> View Knowledge Base Matrix</a></div>
                            <div class="float-end">                            
                                                    ...
                                                    <div class="table-rep-plugin sort_table">
                                                        <div class="table-responsive mb-0">
                                                            <table id="tech-companies-1" class="table table-striped">
                                                                <thead>
                                                                    <tr>
                                                                        <th width="23%"><span><b>Category</b></span></th>
                                                                        <th width="23%"><span><b>Module</b></span></th>
                                                                        <th width="39%"><span><b>Topics</b></span></th>
                                                                        <th width="15%" class="text-center"><span><b>Topic Material</b></span></th>
                                                                    </tr>
                                                                </thead>
                                                                <tbody>
                                                                    <tr>
                                                                        <td>
                                                                            <label class="mt-checkbox mt-checkbox-outline"> <%=categoryName%>
                                                                                <input name="catcb" id="catcb" type="checkbox" value="1" <% if (knowledgebasematrix.checkinlistcat(quest_list, categoryId)) {%>checked<% }%> onchange="javascript: setSubcat('<%=size%>');"/>
                                                                                <span></span>
                                                                            </label>
                                                                        </td>
                                                                    ...
                                                                    <%
                                                                        for (int i = 0; i < size; i++) {
                                                                            KnowledgebasematrixInfo sinfo = (KnowledgebasematrixInfo) list.get(i);
                                                                            if (sinfo != null) {
                                                                                int subcategoryId = sinfo.getDdlValue();
                                                                                String subcategoryName = sinfo.getDdlLabel() != null ? sinfo.getDdlLabel() : "";
                                                                                ArrayList clist = knowledgebasematrix.getListFromList(quest_list, subcategoryId);
                                                                                int clist_size = clist.size();
                                                                                if (clist_size > 0) {
                                                                                    boolean b = knowledgebasematrix.checkinlist(quest_list, subcategoryId);
                                                                    %>
                                                                    <tr>
                                                                ...
                                                                <td>
                                                                    <label class="mt-checkbox mt-checkbox-outline"> <%=subcategoryName%>
                                                                        <input type="checkbox" value="<%=subcategoryId%>" name="subcategorycb" id='subcategorycb_<%=(i + 1)%>' <%=(b == true ? "checked" : "")%> onchange="javascript: setcourse('<%=subcategoryId%>', '<%=(i + 1)%>', '<%=clist_size%>');" />
                                                                        <span></span>
                                                                    </label>
                                                                </td>
                                                                <td class="assets_list">&nbsp;</td>
                                                                ...
                                                                <%
                                                                    for (int j = 0; j < clist_size; j++) {
                                                                        KnowledgebasematrixInfo cinfo = (KnowledgebasematrixInfo) clist.get(j);
                                                                        if (cinfo != null) {
                                                                %>
                                                                <tr>
                                                                <input type="hidden" name="subcategoryId" id="subcategoryId_<%=subcategoryId%>_<%=(j + 1)%>" value="<%= (b == true ? subcategoryId : -1)%>" />
                                                                <input type="hidden" name="questionId" id="questionId_<%=subcategoryId%>_<%=(j + 1)%>" value="<%=(cinfo.getKnowledgebasematrixId() > 0 ? cinfo.getQuestionId() : -1)%>" /> 
                                                                <td class="assets_list">&nbsp;</td>	
                                                                <td class="assets_list">&nbsp;</td>
                                                                <td class="td-center">
                                                                    <label class="mt-checkbox mt-checkbox-outline"> <%=cinfo.getName() != null ? cinfo.getName() : ""%>
                                                                        <input name="questioncb" type="checkbox" value="<%=cinfo.getQuestionId()%>" id="questioncb_<%=subcategoryId%>_<%=(j + 1)%>" <%=(cinfo.getKnowledgebasematrixId() > 0 ? "checked" : "")%> onchange="javascript: setquestionhidden('<%=subcategoryId%>', '<%=(j + 1)%>');" />
                                                                        <span></span>
                                                                    </label>
                                                                ...
                                                                <td class="action_column text-center"><% if (cinfo.getCount() > 0) {%><a href="javascript:;" data-bs-toggle="modal" data-bs-target="#view_resume_list" class="" onclick=" javascript : viewimg('<%= cinfo.getQuestionId()%>')"><img src="../assets/images/attachment.png"></a><%} else {%>&nbsp;<%}%></td>
                                                                </tr>
                                                                <%
                                                                                    }
                                                                                }
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <h3>Knowledge Base Matrix Configured</h3>
                                    <p>Selected Topic(s) have been assigned to the position.</p>
                                    <a href="javascript: showDetailBack();" class="msg_button" style="text-decoration: underline;">Knowledge Base Matrix</a>
                                </div>
                            </div>
                        </div>
                    </div>
