...
<%@page import="com.web.jxp.grade.GradeInfo"%>
<%@taglib uri="/WEB-INF/struts-html-el.tld" prefix="html"%>
<%@taglib uri="/WEB-INF/fmt.tld" prefix="fmt"%>
<%@taglib uri="/WEB-INF/c.tld" prefix="c"%>
<jsp:useBean id="grade" class="com.web.jxp.grade.Grade" scope="page"/>
<!doctype html>
<html lang="en">
<%
    try
...
<%
        ...
        int count = 0;
        int recordsperpage = grade.getCount();
        if(session.getAttribute("COUNT_LIST") != null)
            count = Integer.parseInt((String) session.getAttribute("COUNT_LIST"));
        if(session.getAttribute("GRADE_LIST") != null)
            grade_list = (ArrayList) session.getAttribute("GRADE_LIST");
        int pageSize = count / recordsperpage;
        if(count % recordsperpage > 0)
            pageSize = pageSize + 1;

        int total = grade_list.size();
        int showsizelist = grade.getCountList("show_size_list");
        int CurrPageNo = 1;
...
<head>
    <meta charset="utf-8">
    <title><%= grade.getMainPath("title") != null ? grade.getMainPath("title") : "" %></title>
    ...
    <script type="text/javascript" src="../jsnew/grade.js"></script>
    <script type="text/javascript" src="../jsnew/validation.js"></script>
</head>
<body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
    <html:form action="/grade/GradeAction.do" onsubmit="return false;">
    <html:hidden property="doAdd"/>
    <html:hidden property="doModify"/>
    <html:hidden property="doView"/>    
    <html:hidden property="gradeId"/>
        <!-- Begin page -->
...
<%
                                        ...
                                        if (recordsperpage > 0)
                                        {
                                            noOfPages = gradeCount / recordsperpage;
                                            if (gradeCount % recordsperpage > 0)
                                                noOfPages += 1;
                                        }
                                        if(total > 0)
                                        {
%>
                                        <div class="wesl_pagination pagination-mg m_15">
                                            <div class="wesl_pg_rcds">
                                                Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=gradeCount%>)                                                
                                            </div>
...
<%
                                                        ...
                                                        {
                                                            info = (GradeInfo) grade_list.get(i);
                                                            if (info != null) 
                                                            {
                                                                status = info.getStatus();
%>
                                                            <tr class='hand_cursor' href='javascript: void(0);'>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getGradeId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getGradeId()%>');"> <%= grade.getStatusById(info.getStatus()) %>  </td>
                                                                <td>
                                                                    <span class="switch_bth float-end">
                                                                        <div class="form-check form-switch">
                                                                            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getGradeId()%>', '<%=status%>', '<%=i%>');"/>
                                                                        </div>
                                                                    </span>
                                                                    <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getGradeId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                </td>
                                                            </tr>
<%
                                                            }
