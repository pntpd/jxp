...
<html lang="en">
    ...
    <%
            ...
            if(session.getAttribute("APPEALREJECTION_LIST") != null)
                appealrejection_list = (ArrayList) session.getAttribute("APPEALREJECTION_LIST");
    ...
    <head>
        <meta charset="utf-8">
        <title><%= appealrejection.getMainPath("title") != null ? appealrejection.getMainPath("title") : "" %></title>
        ...
        <script type="text/javascript" src="../jsnew/appealrejection.js"></script>
        <script type="text/javascript" src="../jsnew/validation.js"></script>
    </head>
    <body data-sidebar="dark" data-keep-enlarged="true" class="vertical-collpsed">
        <html:form action="/appealrejection/AppealrejectionAction.do" onsubmit="return false;">
            <html:hidden property="doAdd"/>
            <html:hidden property="doModify"/>
            <html:hidden property="doView"/>    
            <html:hidden property="appealrejectionId"/>
            <!-- Begin page -->
                        ...
                        <div class="row head_title_area">
                            <div class="col-md-12 col-xl-12">
                                <div class="float-start back_arrow">
                                    ...
                                    <span>Appeal Rejection Reason</span>
                                </div>
...
%>
                                            <div class="wesl_pagination pagination-mg m_15">
                                                <div class="wesl_pg_rcds">
                                                    Page&nbsp;<%=CurrPageNo%> of <%= noOfPages %>, &nbsp;(<%= nextCount*recordsperpage + 1 %> - <%= last > 0?last:nextCount*recordsperpage + recordsperpage %> record(s) of <%=appealrejectionCount%>)                                                
                                                </div>
...
%>
                                                                <tr class='hand_cursor' href='javascript: void(0);'>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAppealrejectionId()%>');"><%= info.getName() != null ? info.getName() : ""%></td>
                                                                    <td class="hand_cursor" href='javascript: void(0);' onclick="javascript: showDetail('<%= info.getAppealrejectionId()%>');"> <%= appealrejection.getStatusById(info.getStatus()) %>  </td>
                                                                    <td>
                                                                        <span class="switch_bth float-end">
                                                                            <div class="form-check form-switch">
                                                                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault_<%=(i)%>" <% if(status == 1){%>checked<% }%> <% if(!editper.equals("Y")) {%>disabled="true"<% } %> onclick="javascript: deleteForm('<%= info.getAppealrejectionId()%>', '<%=status%>', '<%=i%>');"/>
                                                                            </div>
                                                                        </span>
                                                                        <% if (editper.equals("Y") && info.getStatus() == 1) {%><a href="javascript: modifyForm('<%= info.getAppealrejectionId()%>');" class="edit_mode float-end mr_15"><img src="../assets/images/pencil.png"/></a><% } %>
                                                                    </td>
                                                                </tr>
                                                                <%
                                                                            }
