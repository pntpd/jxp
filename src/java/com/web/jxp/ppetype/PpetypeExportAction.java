...
public class PpetypeExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = ppetype.getListForExcel(search);
            int total = list.size();  
            String fileName = "PPE_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("PPE Type");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            
            cell = row.createCell(c);
            cell.setCellValue("Answer type");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                PpetypeInfo info = (PpetypeInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getTypeId() == 1 ? "Text Field" : "Drop Down Selection");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getTypeId() == 2 ? info.getAddvalue() : "NA");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  ppetype.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);               
                }
                info = null;
            }
