...
public class PcategoryExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            if(type == 1)
            {
                fileName = "Competency_Assessments_List";
                list = pcategory.getExcel(search, assettypeIdIndex, pdeptidIndex); 
            }
            else
            {
                fileName = "Competency_Assessments_Question_List";
                list = pcategory.getQuestionList(pcategoryId, searchq);
            }
            int total = list.size();  
            ...
            {
                PcategoryInfo info = (PcategoryInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    if(type == 1)
                    {
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(pcategory.changeNum(info.getQcount(), 2));
                        cell.setCellStyle(contentstyle);
                    }
                    else if(type == 2)
                    {
