...
public class CrewdayrateExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = crewdayrate.getExcel(search, allclient, permission, cids, assetids, clientIdIndex, assetIdIndex);
            int total = list.size();  
            String fileName = "Crewdayrate_Index_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Pending Rate");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            for(int i = 0; i < total;i++)
            {
                ...
                {                    
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getPendingcount());
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() == 1 ? "Active" : "In-Active");
                    cell.setCellStyle(contentstyle); 
                }
                info = null;
        }
