...
public class BillingcycleExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = billingcycle.getListForExcel( search, allclient, permission, cids, assetids, clientIdIndex, assetIdIndex);
            int total = list.size();  
            String fileName = "Billingcycle_Report";
            ...
            {
                BillingcycleInfo info = (BillingcycleInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    if(info.getTypevalue() != null && !info.getTypevalue().equals(""))
                        cell.setCellValue(info.getTypevalue());
                    else
                        cell.setCellType(cell.CELL_TYPE_BLANK);
                    cell.setCellStyle(contentstyle); 
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getBillingstatus() == 1 ? "Active" : "Inactive");
                    cell.setCellStyle(contentstyle); 
                }
                info = null;
            }
