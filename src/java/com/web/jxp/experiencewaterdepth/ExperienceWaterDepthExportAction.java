...
public class ExperienceWaterDepthExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = experiencewaterdepth.getListForExcel(search);
            int total = list.size();  
            String fileName = "ExperienceWaterDepth_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Unit Measurement");
            ...
            {
                ExperienceWaterDepthInfo info = (ExperienceWaterDepthInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getUnitMeasurement() != null ? info.getUnitMeasurement() : "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getDepth());
                    cell.setCellStyle(contentstyle);
                   
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  experiencewaterdepth.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
        }
