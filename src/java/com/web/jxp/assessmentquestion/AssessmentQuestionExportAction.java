...
public class AssessmentQuestionExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = assessmentQuestion.getExcel(search);
            int total = list.size();  
            String fileName = "AssessmentQuestion_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Assessment Answer Type");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            
            cell = row.createCell(c);
            cell.setCellValue("Assessment Parameter");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                AssessmentQuestionInfo info = (AssessmentQuestionInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getAssessmentAnswerTypeName()!= null ? info.getAssessmentAnswerTypeName(): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getAssessmentParameterName()!= null ? info.getAssessmentParameterName(): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ? assessmentQuestion.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
        }
