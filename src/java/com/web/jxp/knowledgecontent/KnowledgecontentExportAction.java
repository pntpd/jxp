...
public class KnowledgecontentExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            Knowledgecontent createtraining = new Knowledgecontent();
            KnowledgecontentForm frm = (KnowledgecontentForm)form;
            int type = frm.getType();
            ArrayList list = createtraining.getListForExcel();
            int total = list.size(); 
            
            ArrayList modulelist = new ArrayList();
            if(request.getSession().getAttribute("SUBCATEGORY_LIST") != null)
                modulelist = (ArrayList) request.getSession().getAttribute("SUBCATEGORY_LIST"); 
            int size = modulelist.size();
            
            ArrayList topiclist = new ArrayList();
            if(request.getSession().getAttribute("COURSE_LIST") != null)
                topiclist = (ArrayList) request.getSession().getAttribute("COURSE_LIST"); 
            int topic_size = topiclist.size();
            
            String fileName = "KnowledgeContent_Report";
            ...
            if(type ==1)
            {
                ...
                {
                    KnowledgecontentInfo info = (KnowledgecontentInfo) list.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(info.getTopicname()!= null ? info.getTopicname() : "");
                        cell.setCellStyle(contentstyle);
                    }
                    info = null;
            }
            ...
            {
                fileName = "KnowledgeContent_Subcategory";                
                row = sheet.createRow(rowval); 
                ...
                {
                    KnowledgecontentInfo info = (KnowledgecontentInfo) modulelist.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        row = sheet.createRow(rownum++); 
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(info.getCoursecount());
                        cell.setCellStyle(contentstyle);
                    }
                    info = null;
            }
            ...
            {
                fileName = "KnowledgeContent_Topic";
                row = sheet.createRow(rowval); 
                ...
                {
                    KnowledgecontentInfo info = (KnowledgecontentInfo) topiclist.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        row = sheet.createRow(rownum++); 
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(info.getAssettypeNames() != null ? info.getAssettypeNames(): "");
                        cell.setCellStyle(contentstyle);
                    }
                    info = null;
            }
