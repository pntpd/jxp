...
public class DocumentTypeExportAction extends Action
{       
    @Override
       ...
       try
        {
            ...
            ArrayList list = documentType.getListForExcel(search);
            int total = list.size();  
            String fileName = "Required_Document_List_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Module-Sub-Module");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            cell = row.createCell(c);
            cell.setCellValue("Is Mandatory");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                DocumentTypeInfo info = (DocumentTypeInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(documentType.getDocModuleId(info.getDocModule())!= null ? documentType.getDocModuleId(info.getDocModule()) +" - "+ documentType.getDocSubModuleId(info.getDocsubModule()): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(documentType.getDocFormatId(info.getDocFormat())!= null ? documentType.getDocFormatId(info.getDocFormat()): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  documentType.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);                   
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getMflag() == 1 ? "Yse" : "No");
                    cell.setCellStyle(contentstyle);                   
                }
                info = null;
            }
