...
public class BankAccountTypeExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            BankAccountType degree = new BankAccountType();
            BankAccountTypeForm frm = (BankAccountTypeForm)form;         
            String search = frm.getSearch() != null ? frm.getSearch() : "";
            ArrayList list = degree.getListForExcel(search);
            int total = list.size();  
            String fileName = "BankAccountType_Report";
            ...
            {
                BankAccountTypeInfo info = (BankAccountTypeInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  degree.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
