...
public class HoursOfWorkExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = hoursofwork.getListForExcel(search);
            int total = list.size();  
            String fileName = "HoursOfWork_Report";
            ...
            {
                HoursOfWorkInfo info = (HoursOfWorkInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getNoOfHours());
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  hoursofwork.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
