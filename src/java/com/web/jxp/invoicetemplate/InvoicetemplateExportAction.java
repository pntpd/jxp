...
public class InvoicetemplateExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = invoicetemplate.getListForExcel(search, allclient,  permission,  cids,  assetids);
            int total = list.size();  
            String fileName = "Invoicetemplate_Report";
            ...
            {
                InvoicetemplateInfo info = (InvoicetemplateInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  invoicetemplate.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
        }
