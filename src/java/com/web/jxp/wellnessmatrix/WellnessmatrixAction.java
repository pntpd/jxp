...
public class WellnessmatrixAction extends Action
{
    @Override
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        WellnessmatrixForm frm = (WellnessmatrixForm) form;
        Wellnessmatrix wellnessmatrix = new Wellnessmatrix();
        Assettype assettype = new Assettype();
        int count = wellnessmatrix.getCount();
        String search = frm.getSearch() != null && !frm.getSearch().equals("") ? frm.getSearch() : "";
        frm.setSearch(search);
        Collection statuses = wellnessmatrix.getStatuses();
        ...
        Collection clients = wellnessmatrix.getClients(cids, allclient, permission);
        frm.setClients(clients);
        int clientIdIndex = frm.getClientIdIndex();
        frm.setClientIdIndex(clientIdIndex);

        Collection assets = wellnessmatrix.getClientAsset(clientIdIndex, assetids, allclient, permission);
        frm.setAssets(assets);

        int assetIdIndex = frm.getAssetIdIndex();
        frm.setAssetIdIndex(assetIdIndex);
        int check_user = wellnessmatrix.checkUserSession(request, 72, permission);
        if (check_user == -1) {
            return mapping.findForward("default");
        } else if (check_user == -2) {
            String authmess = wellnessmatrix.getMainPath("user_auth");
            authmess = authmess.replaceAll("__MODULE__", "Wellness matrix Module");
        ...
        if (frm.getDoAdd() != null && frm.getDoAdd().equals("yes")) {
            ...
            frm.setStatus(1);
            frm.setWellnessmatrixId(-1);
            Collection assettypes = assettype.getAssettypes();
            frm.setAssettypes(assettypes);
            frm.setMid(wellnessmatrix.changeNum(wellnessmatrix.getMaxId(), 3));
            saveToken(request);
            return mapping.findForward("add_wellnessmatrix");
        }  else if (frm.getDoView() != null && frm.getDoView().equals("yes")) {
            frm.setDoView("no");
            int wellnessmatrixId = frm.getWellnessmatrixId();
            frm.setWellnessmatrixId(wellnessmatrixId);
            int clientId = frm.getClientId();
            int assetId = frm.getAssetId();
            frm.setClientId(clientId);
            frm.setAssetId(assetId);
            WellnessmatrixInfo info = wellnessmatrix.getWellnessmatrixDetailByIdforDetail(clientId,assetId);
            request.getSession().setAttribute("WELLMATRIX_DETAIL", info);
            ArrayList list = wellnessmatrix.getPositionList(clientId, assetId);
            request.getSession().setAttribute("WELLMATRIX_POS_LIST", list);
            return mapping.findForward("view_wellnessmatrix");
            ...
            wellnessmatrix.deleteWellnessmatrixDetail(clientId, assetId, positionId, uId, status);
            ...
            ArrayList wellnessmatrixList = wellnessmatrix.getWellnessmatrixByName(search, 0, count, allclient, permission, cids, assetids,clientIdIndex, assetIdIndex);
            int cnt = 0;
            if (wellnessmatrixList.size() > 0) {
                WellnessmatrixInfo cinfo = (WellnessmatrixInfo) wellnessmatrixList.get(wellnessmatrixList.size() - 1);
            ...
            frm.setWellnessmatrixId(wellnessmatrixId);
            frm.setPositionId(positionId);
            WellnessmatrixInfo info = wellnessmatrix.getPositionInfo(positionId);
            request.getSession().setAttribute("WELLPINFO", info);
            Collection categories = wellnessmatrix.getCategories();
            ...
            WellnessmatrixInfo cidinfo = new WellnessmatrixInfo(categoryId, categoryName);
            ArrayList subcat_list = wellnessmatrix.getSubCategoryList(categoryId);
            ArrayList question_list = wellnessmatrix.getQuestList(clientId, assetId, positionId, categoryId, assettypeId);
            ...
            wellnessmatrix.createWellnessmatrixDetail(categoryId,positionId, subcategoryId, questionId, uId, clientId, assetId);
