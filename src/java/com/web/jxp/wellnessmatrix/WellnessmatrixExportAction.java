...
public class WellnessmatrixExportAction extends Action
{        
    @Override
       ...
       try
        {
            ...
            ArrayList list = wellnessmatrix.getListForExcel(allclient, cids, assetids, clientIdIndex, assetIdIndex);
            int total = list.size();  
            
            ArrayList position_list = new ArrayList();
            if (request.getSession().getAttribute("WELLMATRIX_POS_LIST") != null) 
            {
                position_list = (ArrayList) request.getSession().getAttribute("WELLMATRIX_POS_LIST");
            }
            int position_size = position_list.size();
            
            ArrayList alist = new ArrayList();
            alist = wellnessmatrix.getAssingListForExcel(assetId, categoryId, positionId);
            int alist_size = alist.size();
            
            String fileName = "Wellnessmatrix_Report";
            ...
            if(type == 1)
            {
                ...
                cell = row.createCell(c);
                cell.setCellValue("Wellnessmatrix Name");
            ...
            {
                fileName = "Wellnessmatrix_Details_Report";
                row = sheet.createRow(rowval); 
                ...
                {
                    WellnessmatrixInfo info = (WellnessmatrixInfo) position_list.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        row = sheet.createRow(rownum++); 
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(wellnessmatrix.changeNum(info.getCount1(), 2));
                        cell.setCellStyle(contentstyle); 

                        cell = row.createCell(col++);
                        cell.setCellValue(wellnessmatrix.changeNum(info.getCount2(), 2));
                        cell.setCellStyle(contentstyle); 

                        cell = row.createCell(col++);
                        cell.setCellValue(wellnessmatrix.changeNum(info.getCount3(), 2));
                        cell.setCellStyle(contentstyle); 
                    }
                    info = null;
                }
            ...
            {
                fileName = "Wellnessmatrix_Assign_Report";
                row = sheet.createRow(rowval); 
                ...
                {
                    WellnessmatrixInfo info = (WellnessmatrixInfo) alist.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        row = sheet.createRow(rownum++); 
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(info.getQuestion()!= null ? info.getQuestion(): "");
                        cell.setCellStyle(contentstyle);   
                    }
                    info = null;
                }
