...
public class ContractExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = contract.getListForExcel(search, allclient, permission, cids, assetids);
            int total = list.size();  
            ...
            {
                ContractInfo info = (ContractInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  contract.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);                                          
                }
                info = null;
            }
