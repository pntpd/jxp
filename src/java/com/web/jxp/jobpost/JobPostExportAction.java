...
public class JobPostExportAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try 
        {
            ...
            ArrayList list = jobpost.getListForExcel(search, statusindex, allclient,  permission,  cids,  assetids);
            int total = list.size();
            String fileName = "JobPost_Report";
            ...
            for (int i = 0; i < total; i++) 
            {
                ...
                if (info != null) 
                {
                    ...
                    cell.setCellValue(info.getRefno() != null ? info.getRefno() : "");
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getTargetmobdate()!= null ? info.getTargetmobdate(): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getOpening() != null ? info.getOpening() : "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  jobpost.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
