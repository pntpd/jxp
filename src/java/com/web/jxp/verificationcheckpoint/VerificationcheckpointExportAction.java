...
public class VerificationcheckpointExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            Verificationcheckpoint verificationcheckpointexportaction = new Verificationcheckpoint();
            VerificationcheckpointForm frm = (VerificationcheckpointForm)form;             
            String search =  frm.getSearch() != null ? frm.getSearch() : "";            
            ArrayList list = verificationcheckpointexportaction.getExcel(search);
            int total = list.size();  
            String fileName = "Verification_checkpoint_excel_report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Display Note");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                VerificationcheckpointInfo info = (VerificationcheckpointInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getVerificationcheckpointName()!= null ? info.getVerificationcheckpointName(): "");
                    cell.setCellStyle(contentstyle);
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getDisplaynote()!= null ? info.getDisplaynote(): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getMinverification() == 0 ? "No" : "Yes");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  verificationcheckpointexportaction.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
