...
public class CityExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            City cityexportaction = new City();
            CityForm frm = (CityForm)form;             
            String search =  frm.getSearch() != null ? frm.getSearch() : "";
            
            ArrayList list = cityexportaction.getExcel(search);
            int total = list.size();  
            String fileName = "City_excel_report";
            ...
            {
                CityInfo info = (CityInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(cityexportaction.getStatusById(info.getStatus()));
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
