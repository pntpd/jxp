...
public class CompetencyassessmentsExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            Competencyassessments competencyassessments = new Competencyassessments();
            ...
            ArrayList list = competencyassessments.getExcel(search, departmentId, assettypeIdIndex); 
            int total = list.size();  
            String fileName = "Competencyassessments_excel_report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Schedule Needed");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                CompetencyassessmentsInfo info = (CompetencyassessmentsInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue((info.getSchedule() == 1 ? "Yes" : "No"));
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(competencyassessments.getStatusById(info.getStatus()));
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
