...
public class ManagewellnessExportAction extends Action
{        
    @Override
       ...
       try
        {
            ...
            int total = managewellnessList.size(); 
            if(mode == 1){
            String fileName = "Manage_Wellness_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Total Filled");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            int col = 0;
            ...
            {
                ManagewellnessInfo info = (ManagewellnessInfo) managewellnessList.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getTotal());
                    cell.setCellStyle(contentstyle);                    
                }
                info = null;
            }
            managewellnessList.clear();
           ...
           try (ServletOutputStream os = response.getOutputStream()) {
            ...
            else if(mode == 2)
            {
                ...
                for(int i = 0; i < total;i++)
                {
                    ...
                    {                    
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(info.getQuestCount());
                        cell.setCellStyle(contentstyle);
                        cell = row.createCell(col++);
                        ...
                        cell = row.createCell(col++);
                        cell.setCellValue(info.getRepeatvalue()!= null ? info.getRepeatvalue() : "");
                        cell.setCellStyle(contentstyle);
            ...
            else if (exptype != null && exptype.equals("2"))
            {
                ...
                ManagewellnessInfo info1 = managewellness.getBasicDetail(crewrotationId);
                int positionId = 0;
                if (info1 != null) {
                ...
                for(int i = 0; i < total;i++)
                {
                    ManagewellnessInfo info = (ManagewellnessInfo) assignlist1.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        row = sheet.createRow(rownum++); 
               ...
               try (ServletOutputStream os = response.getOutputStream()) {
                ...
                for(int i = 0; i < total;i++)
                {
                    ManagewellnessInfo info = (ManagewellnessInfo) assignlist2.get(i);
                    if(info != null)
                    {                    
                        col = 0 ;
                        row = sheet.createRow(rownum++); 
