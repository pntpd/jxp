...
public class CassessmentAExportAction extends Action 
{
    @Override
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try 
        {
            ...
            if (request.getSession().getAttribute("LOGININFO") != null) 
            {
                ...
                if (uInfo != null)
                {
                    per = uInfo.getPermission() != null ? uInfo.getPermission(): "";
                    uId = uInfo.getUserId();
            ...
            int assettypeIdIndex = frm.getAssettypeIndex();
            ArrayList list = cassessment.getAssessorListForExcel( search,aPositionIndex, passessmentIndex, assessorIndex, assessorIdn, assettypeIdIndex);
            int total = list.size();
            String fileName = "assessment_assessor_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Score Obtained");
            sheet.setColumnWidth(c, 256 * 30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (info != null) 
                {
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getAssessmentName() != null ? info.getAssessmentName() : "");
                    cell.setCellStyle(contentstyle);
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getAssStatus() != null ? info.getAssStatus() : "");
                    cell.setCellStyle(contentstyle);

                    cell = row.createCell(col++);
                    cell.setCellValue(info.getMinScore());
                    cell.setCellStyle(contentstyle);
