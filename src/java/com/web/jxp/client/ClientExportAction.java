...
public class ClientExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = client.getListForExcel(search, countryIndexId, assettypeIndexId,allclient, permission, cids, assetids);
            int total = list.size();  
            String fileName = "Client_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Person In-Charge");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            for(int i = 0; i < total;i++)
            {
                ...
                {                    
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getInchargename() != null ? info.getInchargename() : "");
                    cell.setCellStyle(contentstyle);
                    ...
                    cell = row.createCell(col++);
                   cell.setCellValue(info.getContact() != null ? info.getContact() : "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);        
                    cell.setCellValue(info.getAssetcount());
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(client.getStatusById( info.getStatus()));
                    cell.setCellStyle(contentstyle);                           
                }
                info = null;
            }
