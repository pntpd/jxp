...
public class PolicyExportAction extends Action
{
    @Override
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
        try
        {
            ...
            Policy policyexportaction = new Policy();
            PolicyForm frm = (PolicyForm) form;
            String search = frm.getSearch() != null && !frm.getSearch().equals("") ? frm.getSearch() : "";

            int clientIndex = frm.getClientIndex();
            int assetIndex = frm.getAssetIndex();
            ArrayList list = policyexportaction.getExcel(search, allclient, permission, cids, assetids,clientIndex,assetIndex);
            int total = list.size();
            String fileName = "Documents_excel_report";
            ...
            {
                PolicyInfo info = (PolicyInfo) list.get(i);
                if (info != null)
                {
                    col = 0;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getPolicyName() != null ? info.getPolicyName() : "");
                    cell.setCellStyle(contentstyle);

                    cell = row.createCell(col++);
                    cell.setCellValue(policyexportaction.getStatusById(info.getStatus()));
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
