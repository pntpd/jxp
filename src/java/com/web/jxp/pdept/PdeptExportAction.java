...
public class PdeptExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = pdept.getExcel(search, assettypeIdIndex, statusIndex); 
            int total = list.size();  
            String fileName = "Pdept_excel_report";
            ...
            {
                PdeptInfo info = (PdeptInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(pdept.getStatusById(info.getStatus()));
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
