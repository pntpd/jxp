...
public class MaillogExportAction extends Action
{
    ...
    { 
       try
        { 
            ...
            ArrayList list = maillog.getListForExcel(search, fromDate, toDate);
            String viewpath = maillog.getMainPath("view_maillog_file");
            int total = list.size();  
            String fileName = "Maillog_Report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("To Address");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            
            cell = row.createCell(c);
            cell.setCellValue("To CC");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                MaillogInfo info = (MaillogInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getEmail() != null ? info.getEmail() : "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getCc() != null ? info.getCc() : "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getSubject()!= null ? info.getSubject(): "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getRegDate() != null ? info.getRegDate() : "");
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue((info.getFilename() != null && !info.getFilename().equals("")) ? viewpath+info.getFilename() : "");
                    cell.setCellStyle(contentstyle);  
                }
                info = null;
        }
