...
public class RejectionreasonExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = rejectionreason.getListForExcel(search);
            int total = list.size();  
            String fileName = "Rejectionreason_Report";
            ...
            {
                RejectionreasonInfo info = (RejectionreasonInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getRejectionType()== 1 ?  "Rejection" : "Declined" );
                    cell.setCellStyle(contentstyle);
                    
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  rejectionreason.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);                    
                }
                info = null;
            }
