...
public class BenefitExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            Benefit benefitexportaction = new Benefit();
            BenefitForm frm = (BenefitForm)form;
             
            String search =  frm.getSearch() != null ? frm.getSearch() : "";
            
            ArrayList list = benefitexportaction.getExcel(search);
            int total = list.size();  
            String fileName = "Benefit_excel_report";
            ...
            cell = row.createCell(c);
            cell.setCellValue("Benefits Types");
            sheet.setColumnWidth(c, 256*30);
            cell.setCellStyle(headstyle);
            c++;
            ...
            {
                BenefitInfo info = (BenefitInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getBenefitName()!= null ? info.getBenefitName(): "");
                    cell.setCellStyle(contentstyle);
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(benefitexportaction.getStatusById(info.getStatus()));
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
