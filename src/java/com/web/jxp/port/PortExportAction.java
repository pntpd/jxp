...
public class PortExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = port.getListForExcel(search);
            int total = list.size();  
            String fileName = "Port_Report";
            ...
            {
                PortInfo info = (PortInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  port.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
