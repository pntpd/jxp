...
public class TctypeExportAction extends Action
{        
    @Override
    ...
    { 
       try
        { 
            ...
            ArrayList list = tctype.getExcel(search, allclient, permission, cids, assetids,clientIndex,assetIndex );
            int total = list.size();  
            String fileName = "Course Type(Create Training)_Report";
            ...
            {
                TctypeInfo info = (TctypeInfo) list.get(i);
                if(info != null)
                {                    
                    col = 0 ;
                    ...
                    cell = row.createCell(col++);
                    cell.setCellValue(info.getStatus() != 0 ?  tctype.getStatusById(info.getStatus()) : "");
                    cell.setCellStyle(contentstyle);
                }
                info = null;
            }
